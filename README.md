...
**版本 `0.0.1`**（首个版本，见 [CHANGELOG](CHANGELOG.md)） · MIT
...
| 栏 | 内容 |
|---|---|
| **① 关卡** | 关卡与文件（本机存档 / 关卡数 / 当前关卡 / 账号 / 活文件 / 地图存档 / 日志局面 / 最近一局）<br>地图体检（名称版本 / 客户端控件条数 / **可创建模板胶囊** / 模板区为空告警） |
...
| **③ 日志** | 局面数 / 最近一局 / **TAG 汇总** / 按 TAG 过滤 / 点胶囊即按该标签过滤 / 日志正文 |
