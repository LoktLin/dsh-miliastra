...
| L1 | 上面九个单元测试（**374** 项） | 部署/探针的字节级行为，Lua 结构校验器不误报也不漏报，截图清理判据不误删、路径守卫不被绕过，**试玩开跑判据不误触发也不连拍**，**按局切分不把两局揉成一局**，**关卡表认不出时如实报行号、几何事实不带判决**，**指标汇总不带判决** | 同上 |
...
**合计 421 项**（374 + L3 31 + 自检 16；不含需要 `dsh web` 在跑的 L4 那 9 项）。
