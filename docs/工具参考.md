...
| `source` | `string` | 否 | —— | op=bind / op=handover：一个 .lua 的**绝对路径**。op=handover 用它**只读**读那一份文件并抽候选交接值（>8 MB / 二进制 / 相对路径一律拒绝，读完零改动）；op=bind 用它当要搬进模拟器的脚本（通常给真机**活文件** .lua；路径随账号/换图变化，别写死）。op=bind 也可以不传它、改用 `script:{path,source}` 直接给源码。 |
