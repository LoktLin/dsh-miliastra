...
> ⚠️ **2026-09-24 复核：QT1–QT5 里 4 条已被后续工作实际落定**（下面逐条标了结果）——
...
- **QT3**（实现）：工具命名空间 —— 引擎搬入后工具是保留对方 6 个 `qxqy_studio_*` 名，还是统一到 `miliastra_*`（自写适配层 ⇒ 我倾向统一前缀 + 文档留别名）。
  → **✅ 已落定**（统一 `miliastra_*`，引擎侧不暴露为工具）。
- **QT4**（范围）：试玩页（FP8）/ 对方 skill（FP9）/ agent 预设（FP9）/ 术语消歧（FP11）各自 in 还是 out。
  → **部分**：试玩页 **in**（`GET /miliastra/play`，v0.2.0 起内嵌进面板）；**对方 skill（`qxqy-simulator`）与 agent 预设（`wonderland-lua-builder`）仍 out**（要搬就得再开工单）。
- **QT5**（实现）：`dsh.client.inject` 只声明真正要用的（`slots`）还是连带升 DSH 版本。
  → **✅ 已落定**（只声明 `slots`，没连带升版本）。
...
| 2026-09-24 | **W2 收尾**（作者：「继续上 W2 我目标是让 ai 更好使用这个插件」）：浏览器试玩页 `GET /miliastra/play`（PixiJS WebGL，复用引擎自带 `PixiPlayRenderer`+`createPlaySession`，与上游同一条浏览器循环）+ `tools/build-sim-play.mjs`（esbuild 打单文件 552 KB **入库**，`--check` 比字节防过期，`prepublishOnly` 也跑）+ 面板入口 | `tests/sim-play-test.mjs` **29/0**（产物一致性、页面约定、内联脚本语法、路由、`files` 放行 HTML、无浏览器协议冒烟）；`npm test` 退出码 0 **521 项** |
