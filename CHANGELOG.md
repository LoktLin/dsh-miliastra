# Changelog

本项目遵循 [语义化版本](https://semver.org/lang/zh-CN/)。

## [0.0.1] — 2026-09-23

首个版本。整套工具链不是设计出来的，是**在原神·千星奇域 7.1 正式服上排一次真实的 bug 排出来的** ——
那次「`game.InstantiateClientUIControl` 对任何索引号都返回 `nil`」折腾了好几小时，
最后定位到「客户端控件模板库为空」。踩过的坑都固化成了工具能力与 README 的「关键知识」。

### Host 半边 · 6 个工具

- **`miliastra_health`** 环境体检：客户端安装 / 关卡 / 活文件 / 地图 / 日志目录定位。
  **两种关卡布局都扫**（`<id>\<id>.gil` 与根目录的 `<id>.gil` —— 只扫前者会漏掉大部分图）。
  另含**编辑器 / 游戏进程状态**（`BeyondEditor.exe` / `YuanShen.exe`，10 秒缓存、best-effort 不抛）。
- **`miliastra_code`** 活文件：`inspect` / `read` / `deploy` / `backup` / `backups` / `restore`。
  部署一律 **先备份 → 二进制拷贝 → 比对 SHA-256 → 校验无 UTF-8 BOM**，任一不满足直接拒绝。
- **`miliastra_map`** 读 `.gil`（protobuf）：`summary` / `clientui` / `script` / `strings`。
  `clientui` 给出**客户端控件谱系 + 可被脚本动态创建的模板清单**；`script` 给出**地图快照 vs 活文件的一致性**。
- **`miliastra_log`** 读 `.gia` 运行时日志：`sessions` / `tail` / `grep` / `tags`，结构化到
  `{时间, 账号, 玩家, 关卡, 正文}`。
- **`miliastra_probe`** 探针模板化：`tree` / `instantiate` / `ping`，`render` → `deploy` → `collect`。
- **`miliastra_echo`** 回显参数，用来确认参数真的传到了 Host。

另外：

- **同源 HTTP 路由** `/miliastra/{status,tools,tool}`，给面板取数据（唯一合法通道）。
  带**本机访问守卫**（非 `127.0.0.1`/`localhost` 一律 403）、未知路由回 404、`Cache-Control: no-store`。
- **系统提示段**：让 agent 知道「有这么个插件、什么时候该用它」，而不是去拼 PowerShell。
- **`/status` 的 `clientHalf` 自检**：查询宿主 `dsh-client-modules` 的 boot 图，
  直接回答「面板 bundle 有没有进 `window.__DSH_BOOT__`」——
  把原先「只能靠人眼看面板出没出来」这件事变成可断言的数据。

### Client 半边 · 侧边栏面板

- 注册到**官方槽位** `sidebar.footer.action`（不是 DOM 注入）
- **三栏**布局（参考蛋仔面板）：① 关卡 / ② 代码 / ③ 日志，每栏独立滚动
- 粉蓝配色；入口图标内联 PNG（像素画，`image-rendering: pixelated`）
- **关卡切换 + 自动跟随**：默认跟着 `miliastra_health` 判定的「当前关卡」走
  （作者在编辑器里换图，面板自己切过去）；点某一关就切成手动锁定，一键回自动
- **进程状态**：免截图看编辑器 / 游戏在不在跑（实测进程名 `BeyondEditor.exe` / `YuanShen.exe`，
  编辑器是**独立进程**不是游戏子进程），并给出「能否试玩」判断（两者都在才行）
- **多活文件支持**：一个关卡可以有多个 `.lua`（不同角色各一个），选择器切换，所有操作跟着走
- **活文件体检**：大小 / 行数 / SHA-256 / **是否带 BOM** / 编码是否合法 / 有无中文注释
- **脚本一致性**：地图里嵌的脚本快照 vs 本地活文件 —— 回答「跑的是不是本地这版代码」
- **备份清单 + 逐条还原**，还原带二次确认与「还原前安全备份」；份数过多只提醒、不自动删
- **探针一键**：选模板（`ping`/`tree`/`instantiate`）→ 部署（**覆盖活文件，带二次确认与自动备份**）
  → 重新试玩 → 「收回结论」直接从最新一局日志里捞该 tag 的输出
- **历史局面**：列出最近 12 局 `.gia`，点某一局只看那一局
- 日志 TAG 汇总 / 按 TAG 过滤（点胶囊即过滤）
- **诊断日志三连**（刻意保留）：`factory 已执行` / `apply 运行中` / `入口已注册` ——
  client 半边失败默认是**全静默**的，这三行让排障一眼定位卡在哪一步

### 修掉的真实缺陷

- **`exports.inject` 没声明 `slots`** → cordis 的服务代理直接抛
  `cannot get property "slots" without inject`，**整条 loader entry 失败**。
  症状最难看：宿主报 `failed to apply loader entry`，而页面上**一条我们的日志都没有**。
  更坑的是当时把诊断日志写在了 `try` 外面 —— 一行探针把整个插件打死了。
- **Client 的 `callTool` 少剥一层信封** → 面板上「本机存档 —」「关卡数 undefined」全是空的。
  信封是三层 `body{ok,data:{name,ok,data:<业务返回体>}}`，这个错**犯了两次**
  （一次在 `live-check.mjs`、一次在面板里）。现在剥离逻辑抽成独立函数并加了回归测试。
- **关卡扫描盲区**：`Beyond_Local_Save_Level` 下有两种布局，只扫带文件夹的那种会**漏掉大部分图**
  （实测 8 个只看见 3 个；修复后 18 个）。
- **`MILIASTRA_LOCALLOW` 写错会静默回退到真实存档根** —— 手滑就会去动真文件。改成「显式指定就照做，指错就明确报错」。
- **备份同秒撞名**：时间戳只到秒，同一秒连部署两次会覆盖前一份备份。改为自动顺延 `-2 / -3`。
- **探针漏 `script:EnableUpdate(true)`** → `OnUpdate` 永不触发，游戏里只打
  `OnInit`/`OnEnable`/`OnStart` 三行空壳。探针模板已内建这行。
- **解析 5 字节 varint 只解 4 字节**，漏掉整整一段 ID 区间（工具早期版本）。

### 部署前的 Lua 结构校验（`lib/lualint.mjs`）

**一段语法错的 Lua 投进沙箱，试玩会静默不生效** —— 脚本根本没起来，日志里只会是「什么都没有」，
这比报错难查得多。所以 `miliastra_code op=deploy` 与 `miliastra_probe op=deploy` 默认先扫一遍结构：

- `lintMode: 'strict'`（默认）—— 缺 `end` / 括号不配平 / 字符串或长注释没闭合 / `repeat` 少了 `until`
  → **拒绝部署，且不碰活文件、不产生备份**
- `lintMode: 'warn'` —— 照投，但问题进 `warnings[]`，不静默吞掉
- `lintMode: 'off'` —— 不校验（逃生舱）；未知取值明确报错，**不静默降级成 off**

零依赖的结构级启发式（不是完整 Lua 解析器）：先严格剥注释与字符串（含 `--[[ ]]` / `[==[ ]==]`），
再统计块开闭与括号配对，报的是**「第 N 行的 function 没有对应的 end」**而不是「文件最后一行」。

- **`miliastra_probe` 新增 `api-surface` 模板**：把 `_G` / `Enum` / `game` / `script` / 元表逐个枚举打出来，
  并对 `KeyEventType` / `KeyboardKeyCode` / `ControllerKeyCode` 逐个实测取值 ——
  用来终结「按键枚举到底是哪张表」这种靠猜的活（模板上线前被结构校验抓出一个 `if … end else … end` 的语法错，
  那种错**投进去就是整段静默失效**）。

### 开发小工具（`tools/`，不随包发布）

- `tools/lint-probes.mjs` —— 把每个探针模板渲染出来逐个结构校验，出错打印上下文行
- `tools/lint-all.mjs <目录>` —— 对整个目录的 `.lua` 跑结构校验

### 修掉的真实缺陷（续）

- **结构校验器把换行丢了** → 「行尾标识符 + 下一行行首词」粘成一个标识符
  （`local sx, sy` 换行接 `local` → `sylocal`），**行首的 `end` 被吞进上一行尾巴**。
  症状极具迷惑性：**真文件恒报「缺 29 个 end」，而单行小样例全绿**。
  逐行拆 token 才抓到（`L130:sylocal`）。现在换行占位保留，`tests/lualint-test.mjs` 有 4 条回归断言盯它。
- **缺 `end` 时报的是「文件最后一行」**，对排障几乎没用。改为记开块栈，报**最内层没关上的那个块**的行号。
- **`api-surface` 模板里的语法错**（`if … then … end else … end`）—— 由新加的模板结构校验抓出。
- **探针长输出被日志静默截断**（2026-09-23 实机跑 `api-surface` 时发现）：
  **单条日志消息上限实测正好 10000 字符**，超出的部分不报错、不提示，直接没了。
  `Enum.KeyEventType`（164 项）那份刚好打满 10000，而排在字母表后面的
  `KeyboardMoveLeftKeyDown` / `KeyboardJumpKeyDown` 恰好全被切掉 ——
  **看日志的人会以为「枚举里没有这个键」**。现在所有长输出走 `pChunked()`，
  按 3500 字符分片并标注 `(1/3)`；测试也加了守卫（模板必须有分片 helper 且 `CHUNK < 10000`）。

### 2026-09-23 实机验证（首次把探针用在真实排障上）

`api-surface` 探针在正式服关卡 `1073741832` 上跑通（部署 → 试玩 → collect → 还原），
带回的运行时事实与**离线官方文档抽取结果逐项对上**：

| 项 | 运行时（探针实测） | 离线文档（`tools/extract-key-enums.mjs`） |
|---|---|---|
| `Enum.KeyEventType` | 164 项 | 164 项 ✅ |
| `Enum.KeyboardKeyCode` | 59 项 | 59 项 ✅ |
| `Enum.ControllerKeyCode` | 25 项 | 25 项 ✅ |
| 画布尺寸 | `1599.9998 x 999.9998` | 文档口径 1600×1000（**注意是浮点，比较要留容差**） |
| 容器节点 | `prefab=1073741866` | 与已知常量一致 ✅ |

同轮还确认：

- **`Enum.KeyboardKeyCode` 里没有物理键名**（没有 `W` / `A` / `Space`），只有语义键
  （`JumpKey` / `MoveLeftKey`…）。所以 `Enum.KeyboardKeyCode.W` **恒为 nil 且不报错**。
  绑键只能用 `Enum.KeyEventType`。
- **`_G` / `Enum` / `game` / `script` 用 `pairs()` 枚举全是 0 项**（宿主对象不暴露 raw 表），
  但 `Enum.<子表>` 可以正常枚举 —— 所以「枚举 API 面」这条路只在 `Enum` 这一层有效。
- `getmetatable(script).__index` 不可枚举；`script.object` 可用（`tostring` = `ClientUIContainerControl:4`）。

### API 字典 + 补间动画清单 + 地图体检扩展 + `api-check` 探针（2026-09-23，作者要求「探测下还有啥功能可以被使用」）

先用探测把「还有哪些能力没被用起来」摊开，四处发现各做了一件：

**① 把官方 API 文档抽成可查字典**（`docs\千星奇域_API参考.md`）
官方原文是**整页压成一行**（35 KB），翻不到。新工具 `tools\extract-api-reference.mjs` 抽取：
**153 个接口 / 函数 / 生命周期回调 + 27 张枚举表 / 394 个成员**，按接收者分组、带参数/返回值/说明。

> 抽取过程里踩到两个正则坑，都写进注释了：
> · 枚举成员在原文里是**表名直接拼成员名**（`Enum.EaseTypeLinear`），不是 `Enum.EaseType.Linear`
>   —— 必须先找表声明再切段，否则一张表都抽不出来；
> · 参数里会**嵌套括号**（`onComplete: fun()`），用 `[^)]*` 会漏掉一批条目
>   （`Tween:SetOnComplete` / `TweenSequence:AppendCallback` 全被上一条吞进说明里）。
> 另外我在对话里给作者报过「479 函数 / 420 枚举」——**那个数字是错的**（把枚举成员当函数数了），
> 校准后的真实数字是 153 / 27。教训：报数之前先用正则量一遍。

**② 补间动画能力清单**（`docs\千星奇域_补间动画.md`）
文档里自带一整套**没用过**的动画 API：`game.Tween` + 11 个方法、`game.TweenSequence` + 15 个方法、
`Enum.EaseType` **31 条缓动曲线**、以及 `Color.FromRGB/FromRGBA/ToRGBA`。
同页还有 `game.PauseLevelTime` / `PlayAudio2D` / `GetText`（本地化）/ **`game.IsTestPlay`**。

**③ 地图体检扩展**（静态读 `.gil`，不用试玩、不碰任何文件）
`lib/gil.mjs` 新增三个读取器，接进 `miliastra_map op=summary` 与面板「地图体检」卡片：

| 字段 | 读出来是什么 | 实测（本机地图） |
|---|---|---|
| `#29` | 客户端 + 资源版本 | `CNRELWin7.1.0`　资源 `48145775 / 48379043 / 48455349 / 48474090` |
| `#11` | 阵营 / 出生点 / 预设点 | 阵营 3 个（初始玩家/物件/造物阵营 + `UI_MarkPlayer_Faction_*`）、出生点「出生点1」、预设点「新建预设点」 |
| `#7` | 场景对象 | **403 条**（`地形01` …） |

> `#11` 的形状不规整：「出生点」那条记录本身就在 `#11.#3.#2` 里，而「预设点」在 `#11.#5.#1` 里又是一层列表。
> 写死路径会两头都读空（**踩过**：出生点/预设点都读成 0 个）→ 改成递归找 `#501`，两种形状都能吃下。

**④ 新增探针模板 `api-check`（核文档）**
把文档抽出来的接口名逐个在**真机**上按名取一次，报「存在 / 取不到」；顺带打印
`game.IsTestPlay()`（= 官方口径的「是不是在试玩」）、`GetStageMode()`、语言、画布，
并**实测一次 `game.Tween`**（空目标表、0.01 秒、立刻 `Kill` —— 只问一句，不动画面）。

> 这就是 AGENTS.md 铁律 5「API 不许猜」的落地：**文档查名字 → 真机核存在**。
> 状态：`api-check` 已在真机跑过（见下），文档抽出来的 `EaseType` 等仍标 `pending`。

新增开发工具：`tools\gil-field.mjs`（展开 `.gil` 任意顶层字段）、`tools\gil-info.mjs`（一次读完玩法骨架）。

测试：22 + 28 + 10 + 32 + 27 + 9 + 契约自检 16 = **144 项全绿**。

### 面板加「读界面控件」（2026-09-23，作者要求「关键 ui 读取……做到高级功能里面做个样子」）

**Host 侧早就有**（`miliastra_map op=clientui` 读 `.gil` 的客户端控件谱系），
但**面板只露了两个数字**（「客户端控件 N 条 · 可创建模板 M 条」），看不到具体是哪些号。
现在放进「高级诊断」折叠区，做成**样子粗但能直接用**的一版：

```
① 读界面控件（只读 · 不改任何文件）
   [读界面控件]
   可被脚本创建的控件模板（2 个） —— 写 game.InstantiateClientUIControl 就用这些号
     文本框   1073741867
     图片     1073741868
   容器节点（3 个）：1073741852 / 1073741859 / 1073741863
     —— 画布上摆的容器实例，不能被脚本创建（但脚本要挂东西时认的就是这类号）
   [展开全部 37 条记录]
   提示：「无父节点」只是「候选」条件 …… 想确证某个号能不能创建，用探针「试钥匙」。
② 探针（⚠️ 会临时覆盖你的脚本 —— 主要给 AI 排障用）
```

为什么值得放在面板上：写客户端脚本最缺的两个号就是**容器节点索引**与**控件模板索引**，
以前得靠人抄、或者写探针去试。这条是**静态读取**（读地图存档），**不用试玩、不碰任何文件** ——
所以它和「会覆盖脚本的探针」放在同一个折叠区里，但**风险分开标注**。

同时把「高级诊断」里两块的风险讲清楚（只读 vs 会覆盖），
并把「界面文案不许出现 Markdown 记号」的测试**从 1 种渲染态扩到 3 种**（收起 / 展开 / 展开且带数据）——
扩完立刻又抓到两处漏网的 `**` 与一处反引号（都是作者会原样看到的字符）。

测试：22 + 28 + 10 + 32 + 27 + 9 + 契约自检 16 = **144 项全绿**。

### 活文件安全加固（2026-09-23，作者要求「再查查会不会把用户的 lua 给吃了」）

活文件是**唯一副本**：它只活在米哈游的本地存档目录里，没有 git、没有撤销。
按这个前提把「能覆盖 / 能损坏它」的路径逐条审了一遍，**审出 2 个致命 + 4 个高危**，全部修掉：

| # | 级别 | 问题（都实证过） | 修法 |
|---|---|---|---|
| 1 | 🔴 | `deploy` 里**备份失败后没有 return，照样覆盖** | **备份失败立即中止**，活文件零改动，并给出「去检查 _backup 是否可写 / 磁盘是否已满」 |
| 2 | 🔴 | 写文件**非原子**（`copyFileSync` 直写目标）——断电/崩溃会留下半截损坏的文件（备份同样） | 新增 `atomicWriteFile`：同目录临时文件 → **fsync** → **rename 原子替换**（已实测 `renameSync` 在 Windows 上可覆盖） |
| 3 | 🟠 | 备份文件 mtime **沿用源文件**（`copyFileSync` 的坑，已实测） | `listBackups` 改按**文件名里的时间戳**排序（缺了才退回 mtime）；备份 mtime 显式设成创建时间；并列时固定名那份排前 |
| 4 | 🟠 | 备份名时间戳用 **UTC**，比本地少 8 小时 | 改**本地时间**；解析函数 `stampOfName` 的下标 bug 也被测试抓出来了 |
| 5 | 🟠 | `miliastra_probe op=deploy` 把 `_探针_*.lua` 写进**活文件目录**，而「当前文件」按 mtime 取最新 → 探针成了当前文件 | 探针源码改写到**备份目录**（紧挨着被替换的文件，也不再被当成活文件）；另加名字护栏（`pickLuaFile` / `chooseLua` 都会跳过附属文件并报告跳过了几个） |
| 6 | 🟡 | `noBackup:true` 可随手传 → 绕过唯一安全网 | 改成**双钥匙**：必须同时传 `allowNoBackup:true`，否则拒绝 |
| 7 | 🟡 | 没有固定名备份，还原得自己翻时间戳挑 | 每次备份写**两份**：固定名 `<原名>.bak`（还原默认用它）+ 带本地时间戳的历史（永不自动删）；`op=restore` **不传 backup 就用固定名** |
| 8 | 🟡 | 还原失败只报一句 error | 失败给 `nextSteps`；**校验不过自动回滚**到覆盖前那一版并明确告知；部署同理 |
| 9 | 🟡 | 自覆盖（source == dest）没拦 | 拒绝并说明 |

配套的**人的提示**（面板）：备份卡片显示「位置 / 固定名备份 / 备份写在脚本旁边」的说明、
**「还原到最新备份」一键按钮**（不用挑版本）、列表里固定名那份标 `★`、
覆盖前确认语写明「覆盖谁 + 会先备份 + 校验不过会自动回滚」、失败时把 `nextSteps` 一起显示。

配套的**AI 提示**（工具描述）：工具描述里单列「安全约定（写活文件的地方都遵守，别绕过）」7 条；
`file` / `backup` / `noBackup` / `allowNoBackup` / `backupDir` 每个参数都写清后果；
所有写操作回执 `restoreWith`（照着跑就能还原）。

测试：22 + 28 + 10 + 32 + 26 + 9 + 契约自检 16 = **143 项全绿**。
其中新增 12 条安全用例（备份失败不覆盖 / 原子写无 tmp 残留 / 固定名还原 / 回滚 / 双钥匙 / 自覆盖 / 附属文件跳过 …）。

### 试玩结束自动取回最新一局（2026-09-23，作者要求）

面板右栏「运行时日志」新增**「试玩完自动取：开」**（默认开）：守望 `.gia` 文件，
新的一局出现、或文件还在变大 → 说明试玩在动；安静 6 秒后**自动取回那一局**，不用再点「取日志」。

判据是个纯函数（`autoFollowStep`），因为**定时器不好测、判据好测**：

| 情况 | 行为 |
|---|---|
| 出现新文件 / 文件变大 | 记为「在动」，不取 |
| 还在变大 | 状态显示「这一局还在写（试玩中）」 |
| 安静够 6 秒且「看见动过」 | **自动取回** |
| 打开面板时最新一局是旧的（几小时前） | **不取** —— 别拿旧局面糊用户一脸 |
| 打开面板时最新一局刚写完（30 秒内） | 算「刚玩过」，直接取 |
| 同一版日志（名字+大小没变） | 不重复取 |
| 0 字节 | 不取，显示「这一局还没写出内容」 |

#### 顺手纠正的三个错误认知（都是实测出来的）

之前写文档/代码时想当然了三件事，这次用真数据推翻：

1. **`.gia` 文件名里的那个数字不是游戏进程 pid。** 实测 `YuanShen.exe` 从 12:39 一直开着
   （pid 1088），而文件里的号从 135 逐局涨到 151 —— 它是**每局的会话序号**。
2. **`YuanShen.exe` 在跑 ≠ 在试玩。** 同一个游戏进程跨了当天 13:40~18:44 的**全部**试玩。
   所以「游戏进程活着」不能当作试玩信号（`health.processes` 里的 `canPlaytest` 只表示环境就绪）。
3. **`.gia` 里没有框架级的「开始/结束」记录。** 全字段导出确认：里面**只有脚本自己 `print` 的内容**。
   所以「是否在试玩」没有权威信号，只能用文件动静代理 —— 会滞后约 4~10 秒。

> 想要**零延迟**的唯一办法：让脚本自己在生命周期回调里报（`OnStart` → 打一行 `BEGIN`，
> `OnDisable` → 打一行 `END`）。脚本确实会收到这两个回调（实测 17:04 那局都打出来了）。
> 这需要动用户的脚本，所以没擅自做 —— 已在对话里问过。

新增开发工具 `tools/inspect-gia.mjs`（把一局的 `.gia` 所有记录所有字段原样打出来）——
这次的三个结论就是靠它推翻的：`miliastra_log op=tail` 只回 message 正文，看不见记录本身的形状。

测试：22 + 17 + 10 + 32 + 25 + 9 + 契约自检 16 = 131 项全绿。

### 探针降级 + 日志格式化（2026-09-23，作者反馈「折腾探针对人没啥用啊」）

他是对的。探针解决的是**我（AI）写代码时不知道某件事**这一类问题，而创作者要为它付出
四次手工操作（部署 → 停试玩 → 重开 → 收回 → 还原），换回一堆枚举名。
更尴尬的是那次按键枚举**最后是从离线文档抽出来的**，探针只是复核了一遍。
所以：

- **探针从面板上撤下来**，收进右侧「高级诊断（探针）」折叠卡片，**默认收起**，
  标题上直接挂标签「**AI 专用 · 你最好别碰**」。收起态不渲染内容（不是藏起来），展开才进 DOM。
- 腾出来的位置给**日志**，并把日志从「一坨 `join('\n')`」改成格式化列表：

| 改前 | 改后 |
|---|---|
| 时间/标签/正文同色同字号挤成一行 | 时间（暗）+ `[TAG]`（胶囊）+ 正文（自动折行）分列 |
| 出错那行混在几十行里看不出 | 疑似异常 → 左红竖线 + 底色；警告 → 粉线 |
| 超长行把整块撑爆（枚举 dump 上万字符） | 超 400 字符折叠成「共 N 字符，点开看全」 |
| 高度上限 210px | 52vh（约 3.7 倍），把探针让出的位置吃下来 |
| 无筛选 | 「只看异常」开关 + `N 行 · 疑似异常 X · 警告 Y` 计数 |

> ⚠️ 异常判色是**关键词启发式**（地图脚本 `print` 不带级别），只用来「显眼」，不用来「判定」。
> 已在注释与界面提示里标明，并有反例测试守住 —— `err=nil` 是**正常**输出，不能一见 `nil` 就标红
> （否则「只看异常」全是噪音）。

测试：22 + 17 + 10 + 32 + 23 + 9 + 契约自检 16 = 129 项全绿。

### 把「探针」的说明改成人话（2026-09-23，作者反馈）

原话：**「这探针描述很抽象啊 有没有大白话的说明」「gui 界面也没看懂探针作用」**。
根因是我按**实现者视角**写文案（「只读诊断脚本」「最小连通性」「读画布上所有活控件的控件模板索引并逐个试创建」）。
全改了一轮：

| 位置 | 改前 | 改后 |
|---|---|---|
| 工具描述 | 「探针（只读诊断脚本）模板化」 | 「探针 —— **「问游戏一句」的工具**」，并写明为什么需要它、代价是什么、四步怎么走 |
| 面板小标题 | 只读诊断脚本 | 问游戏一句 |
| 模板按钮 | `ping` / `tree` / `instantiate` | **探活（ping）/ 看控件（tree）/ 试钥匙（instantiate）/ 翻字典（api-surface）** |
| 选中的模板 | 无说明 | 「做什么 / 什么时候点它」两行展开（来自 Host 的 `PROBE_INFO`，单一口径） |
| 面板顶部 | 无 | 「探针是干嘛的 / 什么时候用 / 要付什么代价」+ 四步流程 pills |

同轮修掉的界面问题：

- **面板漏了 `api-surface`**（硬编码了 3 个模板名）→ 改成**从 Host 的 `op=list` 拿清单**，加模板不用改面板；
  拿不到才用面板内的兜底文案，并有测试核对两边不脱节。
- **按钮编号缺「②」**（只有 ① 部署 / ③ 收回，中间那步要人做却没写）→ 补一行显式说明。
- **部署后没有「还原我的脚本」入口** → 新增 **「④ 还原我的脚本」** 一键按钮
  （用部署时自动生成的那份备份；未部署时该按钮不出现）。
- **界面文案里残留 Markdown 记号**（`**粗体**` 在面板上是原样显示的）→ 清掉，并加测试守住。

新增开发工具 `tools/dump-panel-text.mjs` —— 把面板**渲染后的纯文字**打出来。
这次就是没人以使用者视角读过一遍才出的问题，所以把它固化成工具 + 3 条测试
（探针卡片讲人话 / 兜底文案不脱节 / 界面无 Markdown 记号）。

### 工程 / 仓库

- 许可证 **Apache-2.0**；`LICENSE` / `CHANGELOG.md` / `.gitattributes`（统一 LF）/ `.gitignore`
- `files` 白名单（`private` 已移除，npm 可直接发布）；`prepublishOnly: npm test` —— **测试不过发不出去**
- README 含实机截图 `assets/panel.png`（已加进 `files`，否则 npm 详情页是裂图）
- 仓库 <https://github.com/LoktLin/dsh-miliastra>

### 已验证

| 层 | 结果 |
|---|---|
| L1 工具层（`tests/smoke.mjs`） | 22 项 |
| L1 部署与备份 / `lintMode` 三档（`tests/deploy-test.mjs`） | 17 项 |
| L1 探针部署 / 多活文件 / 关卡布局 / 模板结构（`tests/probe-deploy-test.mjs`） | 10 项 |
| L1 Lua 结构校验器 / 15 个真文件回归（`tests/lualint-test.mjs`） | 32 项 |
| L3 真实 React 渲染 + 整面板空状态 SSR（`tests/client-render-test.mjs`） | 15 项 |
| L1 技能契约自检（`dsh-plugin-dev` 的 `selftest.mjs`） | 16 项 |
| **L4 真机**（`tests/live-check.mjs`，对着运行中的 dsh web） | 9 项 |

合计 **121 项**（含自检 16 项）。契约取证自本机 **`dsh 0.1.5-rc.1`**。

其中 L4 的 9 项里有一条值得单独说：**`clientHalf` 断言** ——
它查宿主 `dsh-client-modules` 的 boot 图，机器证明「面板 bundle 已进 `window.__DSH_BOOT__`」
（实测 `dsh-miliastra  63.0 KB  rev=33316704`）。**这一格以前只能靠人眼看面板出没出来。**

### 已知边界

- **编辑器 UI 操作没有自动化通道**：在「客户端控件模板」里点【添加客户端控件】、给容器节点挂脚本、
  建容器节点 —— 这些必须由人在编辑器里完成。本插件替代的是「人和 AI 之间的来回搬运」，不是编辑器操作本身。
- **不做场景写操作**：工具只读，或只写「活文件」（脚本本身），不碰地图数据。
- **负载 / 节点图统计尚未实现**：官方文档确认「负载计算」是编辑器 UI 功能（静态菜单 + 动态试玩报告），
  「节点图」数据虽在 `.gil` 里有痕迹但字段结构尚未逆出。已记录为后续课题。
- **`patchReload: live` 不会重新 import Host 模块**（实测：改 `cordis.patch.yml` 触发重载后
  `/miliastra/status` 仍返回旧字段）—— 所以 **Host 改动一定要重启 `dsh web`**，
  只有 `lib/client.js` 刷新页面即可。
