...
### 探针降级 + 日志格式化（2026-09-23，作者反馈「折腾探针对人没啥用啊」）

他是对的。探针解决的是**我（AI）写代码时不知道某件事**这一类问题，而创作者要为它付出
四次手工操作（部署 → 停试玩 → 重开 → 收回 → 还原），换回一堆枚举名。
更尴尬的是那次按键枚举**最后是从离线文档抽出来的**，探针只是复核了一遍。
所以：

- **探针从面板上撤下来**，收进右侧「高级诊断（探针）」折叠卡片，**默认收起**，
  标题上直接挂标签「**AI 专用 · 你最好别碰**」。收起态不渲染内容（不是藏起来），展开才进 DOM。
- 腾出来的位置给**日志**，并把日志从「一坨 `join('\n')`」改成格式化列表：
...
| 高度上限 210px | 52vh（约 3.7 倍），把探针让出的位置吃下来 |
| 无筛选 | 「只看异常」开关 + `N 行 · 疑似异常 X · 警告 Y` 计数 |

> ⚠️ 异常判色是**关键词启发式**（地图脚本 `print` 不带级别），只用来「显眼」，不用来「判定」。
> 已在注释与界面提示里标明，并有反例测试守住 —— `err=nil` 是**正常**输出，不能一见 `nil` 就标红
> （否则「只看异常」全是噪音）。

测试：22 + 17 + 10 + 32 + 23 + 9 + 契约自检 16 = 129 项全绿。

### 把「探针」的说明改成人话（2026-09-23，作者反馈）

原话：**「这探针描述很抽象啊 有没有大白话的说明」「gui 界面也没看懂探针作用」**。
根因是我按**实现者视角**写文案（「只读诊断脚本」「最小连通性」「读画布上所有活控件的控件模板索引并逐个试创建」）。
全改了一轮：

| 位置 | 改前 | 改后 |
|---|---|---|
| 工具描述 | 「探针（只读诊断脚本）模板化」 | 「探针 —— **「问游戏一句」的工具**」，并写明为什么需要它、代价是什么、四步怎么走 |
| 面板小标题 | 只读诊断脚本 | 问游戏一句 |
| 模板按钮 | `ping` / `tree` / `instantiate` | **探活（ping）/ 看控件（tree）/ 试钥匙（instantiate）/ 翻字典（api-surface）** |
| 选中的模板 | 无说明 | 「做什么 / 什么时候点它」两行展开（来自 Host 的 `PROBE_INFO`，单一口径） |
| 面板顶部 | 无 | 「探针是干嘛的 / 什么时候用 / 要付什么代价」+ 四步流程 pills |

同轮修掉的界面问题：

- **面板漏了 `api-surface`**（硬编码了 3 个模板名）→ 改成**从 Host 的 `op=list` 拿清单**，加模板不用改面板；
  拿不到才用面板内的兜底文案，并有测试核对两边不脱节。
- **按钮编号缺「②」**（只有 ① 部署 / ③ 收回，中间那步要人做却没写）→ 补一行显式说明。
- **部署后没有「还原我的脚本」入口** → 新增 **「④ 还原我的脚本」** 一键按钮
  （用部署时自动生成的那份备份；未部署时该按钮不出现）。
- **界面文案里残留 Markdown 记号**（`**粗体**` 在面板上是原样显示的）→ 清掉，并加测试守住。

新增开发工具 `tools/dump-panel-text.mjs` —— 把面板**渲染后的纯文字**打出来。
这次就是没人以使用者视角读过一遍才出的问题，所以把它固化成工具 + 3 条测试
（探针卡片讲人话 / 兜底文案不脱节 / 界面无 Markdown 记号）。
