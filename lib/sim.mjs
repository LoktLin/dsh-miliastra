...
export const PATCH_OP_FIELDS = {
  ...
  set: ['op', 'id', 'key', 'value', 'expectedRevision'],
  remove: ['op', 'id', 'expectedRevision'],
  setCanvas: ['op', 'canvasId', 'expectedRevision'],
  select: ['op', 'id', 'expectedRevision'],
  pick: ['op', 'x', 'y', 'expectedRevision'],
  reparent: ['op', 'id', 'parentId', 'expectedRevision'],
  moveSibling: ['op', 'id', 'direction', 'expectedRevision'],
  ...
  if (op === 'set') {
    ...
    if (unknown.indexOf('field') >= 0) {
      throw new Error('`set` 需要 {op, id, key, value}；你传了 `field` —— 是不是想传 `key`？'
        + '（`path` 只有脚本类 op 用，例如 updateScript / removeScript）' + keysHint(settableKeys));
    }
    if (unknown.length) {
...
export async function simOp(args = {}, ctx = {}) {
  ...
  if (op === 'patch') {
    ...
    if (rawOp === 'set') settableKeys = settableKeysOf(controller, patch.id);
    const v = validatePatchFields(patch, { settableKeys });
    /*
