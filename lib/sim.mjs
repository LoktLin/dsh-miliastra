...
export async function simOp(args = {}, ctx = {}) {
  ...
  if (op === 'bind') {
    ...
    const templates = normalizeBindTemplates(args.templates);
    const script = await readBindScript(args);
    ...
    const fresh = args.fresh !== false;          // 默认：先把存档重置成干净状态，再重建 ⇒ 同一份参数 → 同一份工程
    const keepFactory = args.keepFactory === true; // 保留出厂橱窗控件（默认清掉，否则分不清哪些是你的）
    ...
    const handover = checkHandover(script.source, templates, args.containerId);
    ...
    if (args.saveAs) {
      const name = typeof args.saveAs === 'string' ? args.saveAs : ('bind-' + script.stem + '.save.json');
