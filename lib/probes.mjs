...
/**
 * **自定义探针**的模板名：正文由调用方给（`lua` 参数），所以它**不在** `PROBE_TEMPLATES` 里 ——
 * 那一条清单的语义是「不带参数就能渲染」，把 custom 混进去会让「渲染一个空探针」变成可能。
...
export const PROBE_INFO = {
  ...
  [CUSTOM_TEMPLATE]: {
    ...
    what: '把你自己写的一段 Lua（**要定义 <code>function run()</code>**，里面用 print 或 p() 输出）套上探针的前后缀：'
    ...
    when: '要问的问题两个现成模板都答不了时（例如「OnInit / OnEnable 期能不能 InstantiateClientUIControl」「锚点是不是归一化 0..1」）—— '
      + '**只读**：探针不做场景写操作，只会 print。正文会先过一遍结构校验，缺 end / 括号不配平会被**拒绝渲染**。',
  },
  'api-check': {
    label: '核文档',
 ...
 */
export function renderProbe(template, { tag = 'PROBE', ids, from, to, liveMax, lua } = {}) {
  ...
  if (key === CUSTOM_TEMPLATE) {
    ...
    if (!src.trim()) {
      return {
        ...
        error: '模板 custom 需要 `lua`（探针正文，一段完整 Lua）—— 例：'
    ...
    if (!whole.ok) {
      return {
        ok: false, template: key, tag: safeTag, templates: PROBE_TEMPLATE_CHOICES,
        error: '拼上探针的前后缀之后结构不合法（常见原因：正文里定义了同名的块，或最后少了 end）：' + lintSummary(whole),
        problems: whole.problems, stats: whole.stats,
      };
    }
    const warnings = [];
    if (!/function\s+run\s*\(/.test(src)) {
      warnings.push('正文里没有 `function run()` —— 探针会在试玩起来后第 3 帧调用 run()，'
  ...
  const luaText = PRELUDE(safeTag, key) + body({ ids, from, to, liveMax }) + EPILOGUE;
