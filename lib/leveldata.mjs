...
/* ------------------------------------- ④ 找关卡表（`local LEVELS = {` / `DATA.LEVELS = {`） */

/**
 * 「文件里像关卡表的声明 / 赋值」的一条候选。
 * @typedef {{name:string, kind:'local'|'field'|'assign', line:number}} LevelNameCandidate
 */
/** 正则转义（`nameHint` 是调用方给的，不能直接拼进 RegExp）。 */
const escapeRe = (s) => String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

/**
 * 找「关卡表的赋值位点」。**两种写法都认**（2026-09-25 补第二种 —— 同事的活文件就是这么写的）：
 *   ① `local LEVELS = { … }`（本机 `双相.lua` 的写法）
 *   ② `DATA.LEVELS = { … }`（**字段赋值**；`M.DATA.LEVELS` 这种多级前缀也认，裸赋值 `LEVELS = {` 落在这一档）
 *
 * 为什么必须两种都认：只认 ① 时，同事那种 `DATA.LEVELS = {` 会**抽不到表**，而报错只说
 * 「没找到 `local LEVELS = {`」—— 他既不知道工具认哪些写法，也不知道该传什么 `nameHint`。
 * 两种都命中时取**文件里靠前**的那个（同一份文件里不会有两个关卡表）。
 */
function findLevelTableAt(clean, nameHint) {
  const h = escapeRe(String(nameHint || 'LEVELS').trim() || 'LEVELS');
  const hits = [];
  // ① `local LEVELS = {`
  const localRe = new RegExp('^[ \\t]*local\\s+(' + h + ')\\s*=\\s*\\{', 'm');
  const lm = localRe.exec(clean);
  if (lm) hits.push({ kind: 'local', name: lm[1].trim(), index: lm.index });
  // ② `DATA.LEVELS = {`（前缀是标识符链；最后一节必须正好是 nameHint）
  const fieldRe = new RegExp('^[ \\t]*((?:[A-Za-z_][A-Za-z0-9_]*[ \\t]*\\.\\s*)*' + h + ')\\s*=\\s*\\{', 'm');
  const fm = fieldRe.exec(clean);
  if (fm) hits.push({ kind: 'field', name: fm[1].trim(), index: fm.index });
  hits.sort((a, b) => a.index - b.index);
  return hits[0] || null;
}

/**
 * 「文件里像关卡表的声明 / 赋值」——**抽不到表时给候选**，让人/AI 一眼知道该传什么 `nameHint`。
 *
 * 纯函数（可单测）：只按**形状**扫（`local X = {` / `A.X = {` / `X = {`），不猜语义。
 * 排序把「最像关卡表」的排前面（名字等于 / 结尾是 / 含 nameHint，或带 LEVEL/STAGE/关卡）。
 * 只报形状事实，**不替人判断**哪一条才是关卡表。
 *
 * @returns {{rows: LevelNameCandidate[], total:number}}
 */
export function findLevelTableCandidates(clean, { nameHint = 'LEVELS', limit = 12 } = {}) {
  const want = String(nameHint || 'LEVELS').trim().toLowerCase() || 'levels';
  const rows = [];
  const re = /^[ \t]*(local[ \t]+)?((?:[A-Za-z_][A-Za-z0-9_]*[ \t]*\.[ \t]*)*[A-Za-z_][A-Za-z0-9_]*)[ \t]*=[ \t]*\{/gm;
  let m;
  while ((m = re.exec(String(clean))) !== null) {
    const name = m[2];
    rows.push({
      name,
      kind: m[1] ? 'local' : (name.includes('.') ? 'field' : 'assign'),
      line: String(clean).slice(0, m.index).split('\n').length,
    });
  }
  const score = (r) => {
    const n = r.name.toLowerCase();
    if (n === want || n.endsWith('.' + want)) return 0;
    if (n.includes(want)) return 1;
    if (/level|stage|关卡/.test(n)) return 2;
    return 3;
  };
  rows.sort((a, b) => score(a) - score(b) || a.line - b.line);
  return { rows: rows.slice(0, limit), total: rows.length };
}

/**
 * 从源码里抽出关卡表。返回：
 *   `{ ok:true, levels, constants, matched:{name,kind}, blockLines }`
 *   `{ ok:false, error, line?, nameCandidates, searchedFor, … }`
 *
 * `nameHint` 默认 `LEVELS`；可按需换成别的变量名（**抽不到时回执里会列候选**，见 `findLevelTableCandidates`）。
 *
 * @param {string} src 活文件全文
 * @param {{nameHint?: string}} [opts] `nameHint` = 关卡表的变量名（默认 `LEVELS`）
 * @returns {{ok:boolean, levels?: any[], constants: any[], matched?: {name:string, kind:string},
 *            blockLines?: {from:number, to:number}, error?: string, line?: number, lineText?: string|null,
 *            searchedFor?: string, nameCandidates?: LevelNameCandidate[], nameCandidatesTotal?: number, hint?: string}}
 */
export function extractLevelTable(src, { nameHint = 'LEVELS' } = {}) {
  ...
  const hint = String(nameHint || 'LEVELS').trim() || 'LEVELS';
  const candidates = () => {
    const c = findLevelTableCandidates(clean, { nameHint: hint });
    return { nameCandidates: c.rows, nameCandidatesTotal: c.total };
  };

  const found = findLevelTableAt(clean, hint);
  if (!found) {
    return {
      ok: false,
      error: '活文件里没有找到关卡表（`local ' + hint + ' = {` 或 `X.' + hint + ' = {`）'
        + ' —— 关卡表可能改名了，或者用的是别的写法',
      constants: constList,
      searchedFor: hint,
      ...candidates(),
  ...
  const braceAt = clean.indexOf('{', found.index);
  ...
  if (!Array.isArray(value)) {
    return {
      ok: false,
      error: '`' + found.name + '` 解析出来不是数组（顶层应当是 `{ 关卡1, 关卡2, … }`）',
      line: lineAt, constants: constList, searchedFor: hint,
      ...candidates(),
      hint: '关卡表顶层必须是数组（`{ 关卡1, 关卡2, … }`）—— **单个关卡对象不会被当成一关**；'
        + '若你的表是「名字段」或 `{ [1]=…, [2]=… }` 这种形状，这个工具不适用。',
    };
  }
  return {
    ok: true, levels: value, constants: constList,
    matched: { name: found.name, kind: found.kind },
    blockLines: { from: lineAt, to: lineAt + clean.slice(braceAt).split('\n').length },
  };
}
...
/**
 * 一关的**一行摘要** —— 给「先扫一眼再钻进去」用。
 *
 * 为什么单独有它：`describeLevels` 一关要带上每块平台的坐标，3 关就是 **~20 KB**（≈5k tokens）。
 * 想先看「哪一关值得看」时，那些坐标是纯负担。摘要只有十几个数字，3 关 **~1 KB**。
 *
 * ⚠️ 同样**只有数字、没有判决词**。
 */
export function levelSummary(card) {
  const f = card.facts || {};
  const adj = Array.isArray(f.adjacent) ? f.adjacent : [];
  return {
    stage: card.index,
    name: card.name,
    platCount: card.platCount,
    unparsedPlats: card.unparsedPlats || 0,
    kinds: f.kinds || {},
    spanX: f.spanX,
    spanY: f.spanY,
    adjacentCount: adj.length,
    adjacentOverlaps: adj.filter((a) => a.overlapped).length,
    risingOverlaps: (f.risingOverlaps || []).length,
    trueOverlaps: (f.overlaps || []).length,
    nearMiss: (f.nearMiss || []).length,
    isoCount: f.isoCount,
  };
}
