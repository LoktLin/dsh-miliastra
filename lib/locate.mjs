      ...
      for (const levelId of listDirs(saveRoot)) {
        ...
        if (fs.existsSync(luaDir)) {
          for (const n of (() => { try { return fs.readdirSync(luaDir); } catch { return []; } })()) {
            ...
            if (!st) continue;
            luaFiles.push({ name: n, path: path.join(luaDir, n), size: st.size, mtime: st.mtime.toISOString(), mtimeMs: st.mtimeMs });
...
/** 同一 levelId 的全部副本（跨账号）。 */
export function findLevelAll(levels, q) {
  if (!q) return [];
  const s = String(q);
  return levels.filter((l) => l.levelId === s);
}

/** 找「原神」主进程（编辑器/游戏都叫 YuanShen.exe）。 */
export function findClientProcess() {
  // 不引第三方依赖：走 tasklist 太慢且会弹控制台，这里只做「进程名是否存在」的轻量判断由调用方决定
  return null;
}
