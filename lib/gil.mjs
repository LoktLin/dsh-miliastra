 ...
 *   · 拿到了地图里嵌的脚本名（`mountedNames` 非空）→ **能判断**：
 *       名字对得上 = `mounted:true`；对不上 = `mounted:false`（挂的是别的脚本）；
 *   · 拿不到（没有 .gil / 解析失败 / 地图里没有脚本映射记录）→ `known:false`，
 *     如实说「**判断不了**」，**不猜**（既不说挂了、也不说没挂）。
 *
 * @param {{mountedNames?: string|string[]|null, liveName?: string|null}} [input]
 *        `mountedNames` = 地图存档里嵌的脚本名候选（`file` / `name` 各给一个也行）
 * @returns {{known:boolean, mounted:boolean|null, source:string, matched:string|null,
 *            mountedNames:string[], note:string}}
 */
export function mountStatusOf({ mountedNames = [], liveName = null } = {}) {
  ...
  if (hit) {
    return {
      known: true, mounted: true, source: 'gil-script-name', matched: hit, mountedNames: names,
      note: '地图存档里嵌的脚本名（' + hit + '）与这份对得上 —— 编辑器已经把它挂到这个关卡上了',
    };
  }
  return {
    known: true, mounted: false, source: 'gil-script-name', matched: null, mountedNames: names,
    note: '地图存档里嵌的脚本名是 ' + names.join(' / ') + '，与这份（' + live + '）对不上',
  };
}
