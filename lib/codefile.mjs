 ...
 * @param {object} [opts]
 * @param {string} [opts.backupDir] 备份目录
 * @param {boolean} [opts.noBackup] 跳过备份（危险）
 * @param {'strict'|'warn'|'off'} [opts.lintMode='strict']
 *        Lua 结构校验：strict=不通过就拒绝部署／warn=只带提示／off=不校验
 * @returns {{ok:boolean, dest:string, bytes:number, sha256:string, verified:boolean,
 *            bomFree:boolean, utf8Ok:boolean, backup:string|null, lint:object, errors:string[]}}
 */
export function deploy(src, dest, { backupDir, noBackup = false, lintMode = 'strict' } = {}) {
  const errors = [];
  const warnings = [];
  if (!fs.existsSync(src)) return { ok: false, errors: ['源文件不存在：' + src], dest };
  if (!['strict', 'warn', 'off'].includes(lintMode)) {
    return { ok: false, errors: ['lintMode 只能是 strict / warn / off，收到：' + JSON.stringify(lintMode)], dest };
  }
  ...
  // Lua 结构校验：把「语法错的脚本投进去 → 试玩静默不生效」挡在部署之前
  ...
  if (errors.length) return { ok: false, errors, warnings, dest, bytes: srcBuf.length, sha256: srcSha, lint };
