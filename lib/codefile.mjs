...
/**
 * `op=read source=<绝对路径>` 的**只读**上限：超过它就拒绝。
 *
 * 为什么要有上限：这条路径是「把一个任意本地文件读进来给 AI 看」——
 * 没有上限时，一份 200 MB 的日志会把回执撑爆（也白占一轮上下文）。
 * 8 MB 是「比任何正常 .lua 都大一到两个数量级」的量级：真超了，多半是给错了文件。
 */
export const MAX_EXTERNAL_READ_BYTES = 8 * 1024 * 1024;

/**
 * 读**任意绝对路径**的 `.lua` 正文（对应 `miliastra_code op=read source=…`）。
 *
 * ★ **只读**：不备份、不校验、不写任何文件。它是「看一眼别人（或自己放在别处）的脚本、
 *   把里面的候选交接值抽出来」的入口 —— **不给任何写 op 开这个口子**：
 *   `deploy` / `restore` / `fixbom` 的 `source` / `backup` 语义**完全不动**。
 *
 * 判据一律「宁可明确拒绝，也不把乱码回给 AI」：
 *   · 相对路径 → 拒绝（面板/工具的运行目录不可预期：插件目录、会话工作区、编辑器目录都可能）
 *   · 不存在 / 是目录 / 是二进制（含 NUL 字节）/ 不是合法 UTF-8 / 超过 `maxBytes`
 *     → 全部回 `{ok:false, error, nextSteps}`，`nextSteps` 是**可照做**的下一步
 *
 * ⚠️ 报错文案**故意不写 Markdown 记号**：它会被面板直接显示给人看（面板不渲染 Markdown）。
 *
 * @param {string} file 绝对路径（`C:\…\x.lua` 与 `C:/…/x.lua` 都认）
 * @param {{head?: number, maxBytes?: number}} [opts] `head` = 预览行数（默认 80，`0` = 全文）
 * @returns {{ok:true, readOnly:true, file:string, bytes:number, lines:number, head:string[],
 *            headCount:number, text:string, truncated:boolean, sha256:string, sha256_12:string,
 *            mtime:string, bom:boolean, utf8Ok:true, note:string}
 *          |{ok:false, error:string, nextSteps:string[]}}
 */
export function readLuaAt(file, { head = 80, maxBytes = MAX_EXTERNAL_READ_BYTES } = {}) {
  const raw = String(file == null ? '' : file).trim();
  if (!raw) {
    return {
      ok: false,
      error: 'op=read source=… 需要一个绝对路径，收到的是空值。',
      nextSteps: ['把 .lua 的完整路径粘过来，例如 C:\\...\\背景图片.lua'],
    };
  }
  /*
   * 只认「带盘符」或「UNC」这两种**真绝对**写法（/ 与 \ 都行）。
   * 为什么不用 `path.isAbsolute`：Windows 上 `/x/y.lua` 与 `\x\y.lua` 会被它判成 absolute，
   * 但那两个其实是「当前盘符下的路径」，解析结果取决于进程当前盘 —— 正是要拒绝的那类不确定性。
   */
  const looksDrive = /^[A-Za-z]:[\\/]/.test(raw);
  const looksUnc = /^[\\/]{2}[^\\/]/.test(raw);
  if (!looksDrive && !looksUnc) {
    return {
      ok: false,
      error: '只接受绝对路径，收到的是相对路径或「当前盘符下的路径」：' + raw,
      nextSteps: [
        '给完整路径（含盘符），例如 C:\\Users\\me\\Desktop\\背景图片.lua —— 正斜杠也可以：C:/Users/me/Desktop/背景图片.lua',
        '面板与工具的运行目录不可预期（插件目录 / 会话工作区 / 编辑器当前目录都可能），所以相对路径会指向别的文件',
        '从资源管理器地址栏复制路径最稳（或 shift + 右键 → 复制文件地址）',
      ],
    };
  }
  const abs = path.win32.normalize(raw);
  let st = null;
  try {
    st = fs.statSync(abs);
  } catch (e) {
    return {
      ok: false,
      error: '文件不存在或读不到：' + abs + '（' + ((e && e.code) || (e && e.message) || '未知原因') + '）',
      nextSteps: [
        '核对路径是否写全（含盘符与 .lua 扩展名），以及是不是从「复制文件地址」里拿的（那种可能带引号）',
        '别把别的机器 / 别的账号下的路径拿过来 —— 用 miliastra_health 看这台机器上的活文件在哪',
      ],
    };
  }
  if (!st.isFile()) {
    return {
      ok: false,
      error: '这不是一个文件（看起来是目录）：' + abs,
      nextSteps: ['给到 .lua 文件本身，而不是它所在的文件夹'],
    };
  }
  if (st.size > maxBytes) {
    return {
      ok: false,
      error: '文件太大，拒绝读取：' + st.size + ' 字节 > 上限 ' + maxBytes + ' 字节（' + (maxBytes / 1024 / 1024) + ' MB）',
      nextSteps: [
        '这多半不是一份 .lua 脚本（检查是不是把日志 / 存档 / 图片给过来了）',
        '确实要读这么大的文本：先截取需要的片段，或换个思路（用对应格式的工具看它）',
      ],
    };
  }
  ...
  if (buf.includes(0)) {
    return {
      ok: false,
      error: '看起来是二进制文件（含 NUL 字节），拒绝按文本读：' + abs,
      nextSteps: ['确认给的是 .lua 源文件；地图存档 / 图片这类请用 miliastra_map 等对应工具'],
    };
  }
  const text = decodeStrict(buf);
  if (text === null) {
    return {
      ok: false,
      error: '不是合法的 UTF-8 文本，拒绝读（继续读只会得到乱码）：' + abs,
      nextSteps: ['确认文件编码（千星脚本要求 UTF-8 无 BOM）', '用编辑器另存为 UTF-8 之后再来读'],
    };
  }
  const all = text.split(/\r?\n/);
  const n = Number.isFinite(head) && head >= 0 ? head : 80;
  const headLines = n === 0 ? all : all.slice(0, n);
  ...
  return {
    ...
    bytes: buf.length,
    lines: all.length,
    head: headLines,                       // 前 N 行（数组）；head=0 时是全部行
    headCount: headLines.length,
    text: headLines.join('\n'),            // 同样的行拼成正文（面板预览直接显示这个）
    truncated: n !== 0 && all.length > n,
    sha256: digest,
    sha256_12: digest.slice(0, 12),
    mtime: st.mtime.toISOString(),
    bom: hasBom(buf),
    utf8Ok: true,
    note: '这是只读读取：不备份、不写入，也不给 deploy / restore / fixbom 开「任意路径」的口子。'
      + ' head = 前 ' + (n === 0 ? '全部' : n) + ' 行，text 是它们拼成的正文。',
  };
}
/* ------------------------------------------------------------------ 备份 */
...
export function deploy(src, dest, { backupDir, noBackup = false, allowNoBackup = false, lintMode = 'strict' } = {}) {
  ...
  /*
   * ★ P2-5（2026-09-26）：目标活文件**原先是不是 0 字节**（或压根不存在）= 「往一个空脚本里第一次写入」。
   *
   * 为什么值得单独标出来：实测创作者新建并挂载 `HUD hud.lua` 后，回执里 `dest` 与候选清单都对，
   * 但**没有任何字段**说明「这是首次写入」—— 而编辑器里那一步「存盘」不做，试玩跑的仍是空脚本，
   * 表现就是「部署了但什么都没发生」。所以首次写入要把这句话显式说出来。
   */
  const destBeforeBytes = beforeBuf ? beforeBuf.length : 0;
  const firstWrite = destBeforeBytes === 0;
  const firstWriteNote = firstWrite
    ? '这是该活文件的**首次写入**（原先 0 字节 / 不存在）—— 编辑器里记得存盘（`.gil` 才嵌得到这份脚本）。'
    : null;
  if (destExists) {
    if (noBackup) {
  ...
  if (verified && !hasBom(destBuf)) {
    return {
      ...
      atomic: true,
      // ★ P2-5：原先 0 字节（或不存在）⇒ 显式说「首次写入 + 编辑器里记得存盘」
      destBeforeBytes, firstWrite, firstWriteNote,
      restoreWith: backup ? restoreCommand(backup, dest) : null,
...
/**
 * 活文件名的**归一化**（只在比较时用，绝不据此改名）：
 * 去首尾空白 → 去掉 `.lua` 后缀 → 再 trim → 转小写。
 *
 * 为什么不用 `path.basename`：`.lua` 这个后缀在**两边都可能缺**
 * （GIL 里 `file:"双相.lua"` 与 `name:"双相"` 并存，人给 `source` 时也可能带或不带）。
 * 大小写：Windows 文件名不区分大小写，但 `===` 区分 —— 实测踩过 `View.lua` / `view.lua`。
 *
 * @param {any} name 文件名（可带路径，只取最后一段）
 * @returns {string}
 */
export function normalizeLiveName(name) {
  return String(name == null ? '' : name).split(/[\\/]/).pop().trim().replace(/\.lua$/i, '').trim().toLowerCase();
}

/**
 * ★ **写盘路径**唯一的目标选择实现（2026-09-26，P0-1）。
 *
 * 为什么要它：`op=deploy` 只给 `source` 时，旧实现**按 mtime 挑目标活文件** ——
 * 实测把 `交互 input.lua` 的内容写进了 `表现 view.lua`（多脚本工程下 10 个 `.lua`，
 * 回执里 `dest` 是 view，`pickedBy:"gil"`）。**活文件是唯一副本**，这种静默写错等于毁数据。
 * 所以写盘这条路径**只认名字**，不认时间：
 *
 *   ① 显式 `file`（最高优先；找不到就**抛错并列出全部候选**）
 *   ② `source` 的 **basename 命中**同名活文件（忽略大小写与 `.lua` 后缀差异）
 *   ③ 目录里**只有一个**非附属活文件 → 允许选中，但标 `basenameMismatch:true` + 给 warning
 *   ④ 其余 → **抛错并列出全部候选**（宁可不干，不许猜）
 *
 * ⚠️ **只读 op 不用它**（`inspect` / `read` / `map op=script` 照旧走 A1 口径
 * 「GIL 挂载名 > mtime」）—— 那些不改任何文件，而且已经在回执里用 `pickedBy` 披露了。
 *
 * @param {{levelId?: any, liveFiles?: Array<{name:string, size?:number, mtime?:any, mtimeMs?:number, auxiliary?:boolean}>,
 *          source?: string|null, file?: string|null}} [input]
 *        `liveFiles` = 该关卡活文件目录里的 `.lua` 清单（顺序无所谓）；`source` = 要投进去的本地文件路径
 * @returns {{picked: Record<string, any>, pickedBy: 'explicit'|'basename'|'only-one', matchedBy: string,
 *            basenameMismatch: boolean, destBasenameMatchesSource: boolean|null,
 *            sourceName: string|null, candidates: Array<{name:string,bytes:number|null,mtime:string|null}>,
 *            warning: string|null, note: string}}
 * @throws {Error} 没命中、且不是「只有一个活文件」那种可解释的情况 —— 报错里**列出全部候选**与可照抄的下一步
 */
export function pickLiveFile({ levelId = null, liveFiles = [], source = null, file = null } = {}) {
  const all = (Array.isArray(liveFiles) ? liveFiles : []).filter((f) => f && typeof f.name === 'string' && f.name);
  // 附属文件（探针源码 / 备份 / 隐藏文件）一律不参选 —— 与选择器护栏同一口径
  const real = all.filter((f) => !f.auxiliary && !isAuxiliaryLuaName(f.name));
  const candidates = real.map((f) => ({
    name: f.name,
    bytes: Number.isFinite(f.size) ? f.size : null,
    mtime: isoMtime(f),
  }));
  const listText = real.map((f) => f.name).join('、') || '（一个都没有）';
  const where = levelId == null ? '这个关卡' : '关卡 ' + levelId;
  const srcName = source ? String(source).split(/[\\/]/).pop() : null;

  const pick = (f, pickedBy, matchedBy) => {
    /*
     * `destBasenameMatchesSource` 是**判据**（不是"选中方式"的复述）：显式 `file` 命中的那一份，
     * 名字**仍可能与 `source` 不同**（实测：source 是 input、file 却写了 view）——
     * 那种情况必须如实报 false + warning，而不是因为"是显式指的"就报 true。
     * 没给 source（只给 file）时无从比较 → null（不猜）。
     */
    const match = srcName ? normalizeLiveName(f.name) === normalizeLiveName(srcName) : null;
    return {
      picked: f, pickedBy, matchedBy,
      basenameMismatch: match === false,
      destBasenameMatchesSource: match,
      sourceName: srcName, candidates,
      warning: match === false
        ? '⚠️ 目标活文件（' + f.name + '）与 source 的文件名（' + srcName + '）**不一样** —— '
          + '确认 `dest` 是不是你要写的那份（它是按 ' + (pickedBy === 'explicit' ? '显式 file' : '唯一候选') + ' 选中的）'
        : null,
      note: pickedBy === 'explicit'
        ? '按显式 file 参数选中（写盘路径**只认名字**，不按「最近改动」猜）'
        : '按 `source` 的文件名命中同名活文件（忽略大小写与 .lua 后缀差异）',
    };
  };
  ...
  if (file != null && String(file).trim()) {
    ...
    const hit = real.find((f) => f.name === want)
      || real.find((f) => normalizeLiveName(f.name) === normalizeLiveName(want));
    if (!hit) {
      throw new Error(where + ' 下没有活文件 "' + want + '"。现有：' + listText
        + '　—— 用 file="<' + '活文件名>' + '" 精确指定（名字必须与目录里那个一致），别让它按「最近改动」猜');
    }
    return pick(hit, 'explicit', 'file');
  }

  // ② source basename 命中
  if (srcName) {
    const hit = real.find((f) => normalizeLiveName(f.name) === normalizeLiveName(srcName));
    if (hit) return pick(hit, 'basename', 'basename');
    // ③ 只有一个非附属活文件 → 允许，但必须**显式标记**（它仍然可能是错的那个）
    if (real.length === 1) {
      const only = real[0];
      return {
        picked: only, pickedBy: 'only-one', matchedBy: 'only-one',
        basenameMismatch: true, destBasenameMatchesSource: false,
        sourceName: srcName, candidates,
        warning: '⚠️ source 的文件名（' + srcName + '）与目标活文件（' + only.name + '）**不一样** —— '
          + where + ' 下只有这 1 个非附属活文件，所以按「唯一候选」选中它；'
          + '如果这不是你要写的那份，**立即用 op=restore 还原**（回执里有 restoreWith），并改用 file="' + only.name + '" 重新部署',
        note: '目录里只有一个非附属活文件 → 按唯一候选选中（basenameMismatch:true）',
      };
    }
    throw new Error('拒绝部署：`source` 的文件名 "' + srcName + '" 在' + where + ' 下**没有同名的活文件**，'
      + '而这里不止一个候选 —— 按「最近改动」猜过会**静默写错文件**（实测把 input.lua 的内容写进了 view.lua），所以宁可不干。'
      + '\n现有活文件（' + real.length + ' 个）：' + listText
      + '\n下一步：把目标活文件的名字显式传给 `file`（例如 {"op":"deploy","source":"…' + srcName + '","file":"' + (real[0] ? real[0].name : '<活文件名>') + '"}）；'
      + '确实要新建一份新脚本时，先在编辑器里给容器节点挂上它（会生成同名活文件），再来部署。');
  }

  throw new Error('选目标活文件需要 `source` 或 `file`（' + where + ' 下现有：' + listText + '）');
}

/**
 * **三方 SHA 对照**（`miliastra_health op=sha`，N-2）—— **纯函数**，只报数字与一句结论。
 *
 * 三列：**活文件**（沙箱里那份，游戏最终读的是它）/ **mirror**（`code/` 之类本地镜像）/ **.gil 嵌入快照**
 * （编辑器存盘时嵌进地图的那份，**试玩真正跑的**是它 ⇒ deploy 之后还差一步「存盘」）。
 *
 * 判据只做「三处哈希相不相同」，**不判谁对谁错**（那取决于你想让哪一版生效）：
 *   · 三方一致 → `三方一致`
 *   · 活文件 ≠ 嵌入快照 → `该存盘了`（部署完了，但编辑器还没把它吃进地图）
 *   · 镜像 ≠ 活文件 → `该备份/该同步镜像了`（本地镜像落后于沙箱，或反过来）
 *
 * @param {{live?: {name?:string, sha256?:string|null, bytes?:number|null}|null,
 *          mirror?: {name?:string, sha256?:string|null, bytes?:number|null, path?:string|null}|null,
 *          embedded?: {name?:string, file?:string|null, sha256?:string|null, bytes?:number|null}|null,
 *          embeddedCount?: number}} [input]
 *         每一项都可以缺（缺 = 这一列**没有可比的对象**，回执里如实给 null，不假装一致）
 * @returns {{rows: Array<{source:string, name:string|null, sha256:string|null, sha256_12:string|null, bytes:number|null, matchesLive:boolean|null}>,
 *            liveSha256: string|null, allThreeMatch: boolean|null, liveVsEmbedded: boolean|null,
 *            liveVsMirror: boolean|null, verdict: string, conclusion: string, caveats: string[]}}
 */
export function compareLiveSources({ live = null, mirror = null, embedded = null, embeddedCount = 0 } = {}) {
  const sha = (x) => (x && typeof x.sha256 === 'string' && x.sha256 ? x.sha256 : null);
  const liveSha = sha(live);
  const mirrorSha = sha(mirror);
  const embeddedSha = sha(embedded);
  const rows = [
    { source: 'live', name: (live && live.name) || null, sha256: liveSha, bytes: live ? (live.bytes ?? null) : null },
    { source: 'mirror', name: (mirror && mirror.name) || null, sha256: mirrorSha, bytes: mirror ? (mirror.bytes ?? null) : null },
    { source: 'embedded', name: (embedded && (embedded.file || embedded.name)) || null, sha256: embeddedSha, bytes: embedded ? (embedded.bytes ?? null) : null },
  ].map((r) => ({
    ...r,
    sha256_12: r.sha256 ? r.sha256.slice(0, 12) : null,
    // 「这一列跟活文件一不一样」——只有两边都有哈希时才判，否则 null（不猜）
    matchesLive: (r.sha256 && liveSha) ? r.sha256 === liveSha : null,
  }));

  const liveVsMirror = (liveSha && mirrorSha) ? liveSha === mirrorSha : null;
  const liveVsEmbedded = (liveSha && embeddedSha) ? liveSha === embeddedSha : null;
  const allThreeMatch = (liveVsMirror === true && liveVsEmbedded === true) ? true
    : ((liveVsMirror === null || liveVsEmbedded === null) ? null : false);

  const caveats = [];
  if (embeddedCount > 1) {
    caveats.push('地图里有 ' + embeddedCount + ' 条脚本映射，`embedded` 只对上了**按名字挑中的那一条** —— '
      + '其它脚本没参与对照（多脚本工程请对每个脚本各跑一次，或看 miliastra_map op=script 的 mappings[]）');
  }
  if (!liveSha) caveats.push('没有活文件（这一列是空的）—— 先用 miliastra_health 确认关卡与活文件目录');
  if (!embeddedSha && embeddedCount === 0) caveats.push('地图里没有脚本快照（编辑器还没存过盘 / 还没挂脚本）');
  if (!mirrorSha) caveats.push('没给镜像目录（`mirror`）或目录里没有同名 .lua —— 只出「活文件 vs .gil」两列');

  let verdict;
  if (liveSha && embeddedSha && liveVsEmbedded === true && liveVsMirror === true) verdict = '三方一致';
  else if (liveSha && embeddedSha && liveVsEmbedded === false) verdict = '该存盘了';
  else if (liveSha && mirrorSha && liveVsMirror === false && liveVsEmbedded === true) verdict = '该同步镜像了';
  else if (liveSha && mirrorSha && liveVsMirror === false) verdict = '镜像与活文件不一致';
  else if (!liveSha || (!embeddedSha && !mirrorSha)) verdict = '比不了';
  else verdict = '只比得出一部分';

  const conclusion = verdict === '三方一致'
    ? '活文件 / 镜像 / 地图嵌入快照**三处哈希相同** —— 游戏跑的就是你手上这一版。'
    : verdict === '该存盘了'
      ? '活文件与**地图里嵌的快照不同** —— deploy 只写了本地活文件，**还差编辑器里存一次盘**（试玩跑的永远是嵌进 .gil 的那份）。'
      : verdict === '该同步镜像了'
        ? '镜像与活文件不同，而活文件与嵌入快照相同 —— 沙箱那份才是最新的，把镜像同步一份（本工具不替你写镜像）。'
        : verdict === '镜像与活文件不一致'
          ? '镜像与活文件哈希不同 —— 两边都可能更新，先看 mtime / 字节数再决定往哪边同步。'
          : verdict === '比不了'
            ? '至少要有一列能比（活文件 + .gil 嵌入快照，或活文件 + 镜像）才出得了结论。'
            : '只比得出一部分（缺某一列的同名对象）—— 看 caveats 补齐。';

  return {
    rows, liveSha256: liveSha,
    allThreeMatch, liveVsMirror, liveVsEmbedded,
    verdict, conclusion, caveats,
  };
}
