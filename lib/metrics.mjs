...
/**
 * 去掉直方图分箱，只留标量 —— `summaryOnly` 用（**纯函数**）。
 *
 * 为什么：`numericStats` 每个键带 10 个分箱，`byLevel` 里再带一份，几个键就上千行。
 * 「先看数」的时候只需要 `n / min / max / median / core / hotBin`。
 * 去掉的只是**分箱**，`core`（集中区）与 `hotBin`（热区）都留着 —— 结论用的正是它们。
 */
export function slimStats(stats) {
  if (!stats || typeof stats !== 'object') return stats || null;
  const { bins, ...rest } = stats;
  // ⚠️ 没有箱子可省时**什么都不加**：加了反而更大 —— 实测单点数据摘要比全量还多 29B（1601→1630），
  // 那就违背了 `summaryOnly` 的本意（只去体积、不去结论）。
  return Array.isArray(bins) && bins.length ? { ...rest, binsOmitted: bins.length } : rest;
}

/** 对 `summarizeMil` 的结果做瘦身（每个事件的 nums 与 byLevel[].nums）。 */
export function slimMil(summaries) {
  return (summaries || []).map((s) => ({
    ...s,
    nums: Object.fromEntries(Object.entries(s.nums || {}).map(([k, v]) => [k, slimStats(v)])),
    byLevel: Object.fromEntries(Object.entries(s.byLevel || {}).map(([lv, d]) => [lv, d.nums
      ? { ...d, nums: Object.fromEntries(Object.entries(d.nums).map(([k, v]) => [k, slimStats(v)])) }
      : d])),
  }));
}

/** 对 `summarizeLoose` 的结果做瘦身（每个数值键）。 */
export function slimLoose(loose) {
  if (!loose) return loose;
  const keys = {};
  for (const [k, st] of Object.entries(loose.keys || {})) {
    const { byLevel, ...rest } = st;
    keys[k] = {
      ...slimStats(rest),
      // ⚠️ `byLevel` 里**每一层也是一份** `numericStats`：以前整块原样带出去，等于只瘦了顶层
      // （实测：顶层少了箱子，`byLevel` 里还各带一份 → 摘要反而更大）。
      ...(byLevel
        ? { byLevel: Object.fromEntries(Object.entries(byLevel).map(([lv, d]) => [lv, slimStats(d)])) }
        : {}),
    };
  }
  return { ...loose, keys };
