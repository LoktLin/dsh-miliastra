...
export function playtestSummary(state, nowMs = Date.now()) {
  ...
  const slim = (r) => ({
    ...
    // ★ 毫秒级时刻也一并给出（0.3.1，反馈 A2/C1）：`.gia` 归属判定、连拍逐张标 inRun 都要拿它算，
    //   只给 `HH:MM:SS` 的话调用方还得自己拼当天日期（实测拼错一次就会判成"上一局"）
    startedAtMs: Number.isFinite(r.startedAtMs) ? r.startedAtMs : null,
    endedAtMs: Number.isFinite(r.endedAtMs) ? r.endedAtMs : null,
