  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      var CSS = [
        ...
        /*
         * 模拟器 tab = **1:2**（作者要求）：左边 1 份是「工程 / 画面日志 / 试玩操作 / 验收单」，
        ...
        '.' + PLUGIN + '-playside{display:flex;flex-direction:column;gap:10px;min-width:0;}',
        '.' + PLUGIN + '-playpane{display:flex;flex-direction:column;gap:8px;min-width:0;}',
        '.' + PLUGIN + '-playhead{display:flex;align-items:center;gap:8px;flex-wrap:wrap;}',
        '.' + PLUGIN + '-playframe{width:100%;height:min(74vh,820px);min-height:420px;border:0;border-radius:10px;',
        'background:#0b1220;box-shadow:0 0 0 1px rgba(125,211,252,.20);}',
        '.' + PLUGIN + '-inline .' + PLUGIN + '-playframe{height:calc(100vh - 190px);}',
        // —— 面板里正文与试玩页**一起滚**（列的内滚在这里让位，避免"滚中滚"） ——
      ...
      function SimulatorBody() {
        ...
        /**
         * ② 画面与日志：**横排**（作者批注）—— 「现在长什么样」和「日志说了什么」一眼同时看到。
         * ⚠️ 这里只是**缩略**（点它也能注入一次点击）；真正的试玩画面在右边那 2/3 的**试玩页**里（iframe，WebGL）。
         */
        ...
        /**
         * ④ **试玩页**（2/3，作者要求）：直接 iframe 嵌 `GET /miliastra/play`（PixiJS WebGL，真能玩）。
        ...
        kids.push(React.createElement('div', { className: PLUGIN + '-simgrid', key: 'grid' },
          React.createElement('div', { className: PLUGIN + '-playside', key: 'L' }, [left, shots, ops, bookCard]),
          playPane));
