  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      var CSS = [
        ...
        // 浮层里的视图切换条（初级功能 / 高级功能 / 全部）
        '.' + PLUGIN + '-viewtabs{display:flex;align-items:center;gap:6px;padding:8px 13px 0;}',
        '.' + PLUGIN + '-vtab{padding:4px 12px;border-radius:999px;border:1px solid rgba(125,211,252,.28);',
        'background:transparent;color:var(--dsw-alias-label-secondary,#8fa6c4);font-size:11.5px;cursor:pointer;}',
      ...
      function Panel(props) {
         ...
         */
        var s29 = React.useState(props.group || 'all');
        ...
        // 浮层里给一条自己的 tab 条：会话区那三个 tab 在头部，第一轮实测「找不到」，所以两处都给
        if (!inline) {
          kids.push(React.createElement('div', { className: PLUGIN + '-viewtabs', key: 'vtabs' },
            ['basic', 'advanced', 'all'].map(function (g) {
              var text = g === 'basic' ? '初级功能' : (g === 'advanced' ? '高级功能' : '全部');
              return React.createElement('button', {
                key: 'vt' + g,
                className: PLUGIN + '-vtab' + (panelTab === g ? ' ' + PLUGIN + '-vtab-on' : ''),
                onClick: function () { setPanelTab(g); },
              }, text);
            }),
            React.createElement('span', { className: PLUGIN + '-hint', key: 'vh', style: { marginLeft: 'auto' } },
              '「模拟器」在会话区顶部那个 tab 里')));
        }
        kids.push(React.createElement('div', { className: PLUGIN + '-body', key: 'body' }, bodyCols));
      ...
      function SimulatorView() {
        ...
        var doPlay = React.useCallback(function (action, a) {
          return engine({ op: 'play', action: action, args: a || {} }).then(function (d) {
            ...
            if (action === 'stop') { setRunning(false); setLogs([]); }
            return d;
          });
        }, [engine]);

        var doShot = React.useCallback(function (target) {
          return engine({ op: 'shot', target: target, label: 'panel' }).then(function (d) {
        ...
        var stage = shot
          ? React.createElement('img', { src: shot.url, alt: 'sim', style: { width: '100%', borderRadius: '6px', background: '#0b1220' } })
        ...
        var actions = React.createElement('div', { className: PLUGIN + '-row', key: 'act' },
          ...
          btn('开始试玩', PLUGIN + '-primary', function () { doPlay('start').then(function (d) { if (d) doShot('play'); }); }, 'b2'),
          btn('单步', null, function () { doPlay('step', { dt: 0.033 }).then(function () { doShot('play'); }); }, 'b3'),
          btn('刷新画面', null, function () { doShot('play'); }, 'b4'),
          btn('停止', null, function () { doPlay('stop'); }, 'b5'),
          btn('居中点击', null, function () {
            var w = (st && st.canvas && st.canvas.width) || 1600;
            var h = (st && st.canvas && st.canvas.height) || 900;
            doPlay('pointer', { type: 'click', x: Math.round(w / 2), y: Math.round(h / 2) })
              .then(function () { doShot('play'); });
          }, 'b6'),
        ...
        var right = React.createElement('div', { className: PLUGIN + '-col', key: 'R' },
          ...
          sec('试玩日志', running ? '运行中' : '未开始',
            React.createElement('div', { className: PLUGIN + '-log', key: 'l', style: { maxHeight: '220px' } },
