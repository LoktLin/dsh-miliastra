  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      function SimulatorBody() {
         ...
         * 这里只留 4 个核心动作；其余（连帧/刷新画面/导出 GIA/浏览器试玩/重置工程/按键/设备）折进「高级」。
         */
        var coreActions = React.createElement('div', { className: PLUGIN + '-row', key: 'act' },
        ...
        var extCands = (extHo && extHo.candidates) || [];
        var extSug = (extHo && extHo.suggestedTemplates) || [];
        var extLuaBlock = React.createElement('div', { key: 'ext', style: { display: 'flex', flexDirection: 'column', gap: '5px', marginBottom: '8px' } },
          ...
          React.createElement('div', { className: PLUGIN + '-hint', key: 't2' },
            '下面那种是沙箱活文件（正在开发的那张图，路径随账号/换图而变）；这条是你手上任意一个 .lua —— '
            + '它不必在沙箱里，读取不会改动它，搭进模拟器也只动模拟器工程'),
              ...
              onClick: function () {
                ...
                Promise.all(reqs.map(function (r) { return callTool(r.name, r.args); }))
                  .then(function (rs) {
                    ...
                    if (Array.isArray(ho.candidates)) {
                      setBindRows(ho.candidates.map(function (c) {
                        return { name: c.name, value: c.value, kind: c.isTemplate ? c.kindHint : '', on: !!c.isTemplate };
                      }));
                    }
                    if (ho.containerId) setBindContainer(String(ho.containerId));
                  })
          ...
          extInfo
            ? React.createElement('div', { key: 'res', style: { display: 'flex', flexDirection: 'column', gap: '4px' } },
              ...
              (extRead && extRead.ok)
                ...
                : null,
              React.createElement('div', { className: PLUGIN + '-hint', key: 'c1' },
                '候选交接值 ' + extCands.length + ' 条（启发式，kind 得你自己确认）：'
                + (extCands.length
                  ? extCands.map(function (c) { return c.name + ' = ' + c.value + (c.kindHint ? '（像 ' + c.kindHint + '）' : ''); }).join('，')
                  : '一条都没有')),
          ...
          React.createElement('div', { className: PLUGIN + '-hint', key: 'sep' }, '—— 或者，从这台机器上的沙箱活文件里挑一份 ——'));
         ...
         * 顺序 = 最常看的在最上面：画面/日志 → 高级操作（折叠）→ 时间线 → 验收单 → 工程（折叠）。
         */
        ...
        /**
         * ② 试玩操作（**高级项折叠**）：面板只留 4 个核心动作（在上面 ①），
         * 连帧 / 按键 / 设备·人数·视角 / 导出 GIA / 重置工程这些试玩页里也有的东西收在这里，默认不占地方。
         */
        var ops = React.createElement('details', { className: PLUGIN + '-sec', key: 'O' },
          React.createElement('summary', { className: PLUGIN + '-sec-title' }, '② 试玩操作（高级：连帧 / 按键 / 设备 / 导出）'),
          React.createElement('div', { className: PLUGIN + '-hint', key: 'i' },
            (running ? '会话运行中' : '未开始') + ' · 帧 ' + statFrame + ' · 控件 ' + statControls
            + ' · 右边试玩页里有同一套控制（都在同一个会话上）'),
          advActions, keyRow, fixedCfgRow);
        ...
        var playsideKids = [shots, ops, histCard, bookCard, engCard];
