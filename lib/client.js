...
(function () {
  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
       ...
       *
       * 踩过两次：`logdiag` 的 `headline/why/next` 与 `BACKUP_NOTE` 里写了 `**强调**`，
       * 面板不渲染 Markdown → 用户看到的是字面量星号。面板不渲染 Markdown，
      ...
      function Panel(props) {
        ...
        /** 「试玩体检」结果（`__logDiag` 仅供渲染测试）。 */
        var s32 = React.useState(props.__logDiag || null); var logDiag = s32[0]; var setLogDiag = s32[1];
        var s33 = React.useState(false); var diagBusy = s33[0]; var setDiagBusy = s33[1];
        /**
         * 试玩结束**自动取回最新一局**（作者要求）。
        ...
        var load = React.useCallback(function (light) {
          ...
          callTool('miliastra_health', { all: true }).then(function (r) {
            ...
            if (r.shots) setShotInfo(r.shots);
        ...
        /**
         * 打开面板 / 切换关卡时自动跑一次「试玩体检」。
         * 目的是**主动**告诉用户「最近一局是不是你刚玩的那局」——
         * 2026-09-23 实机踩到：用户试玩了一局却没有新日志，面板当时安安静静端上旧日志，
         * 没人告诉他漏了哪一步（原因最后是「忘了开」）。
         */
        React.useEffect(function () {
          if (!open) return undefined;
          var alive = true;
          callTool('miliastra_log', Object.assign({ op: 'diagnose' }, activeLevel ? { level: activeLevel } : {}))
            .then(function (r) { if (alive && r && r.ok !== false) setLogDiag(r); })
        ...
        /**
         * 「试玩体检」：问一次「我刚试玩了，为什么没有日志？」
         *
         * 为什么要有它（2026-09-23 实机踩到）：作者试玩了一局，**磁盘上一个新 `.gia` 都没有**，
         * 而面板当时只会安安静静地把上一次的旧日志端上来 —— 用户不知道自己漏了哪一步、
         * 也不知道 `.gia` 根本不是「边玩边写」。所以这里主动把证据摆出来。
         */
        var doDiagnose = function () {
          setDiagBusy(true); setErr('');
          callTool('miliastra_log', Object.assign({ op: 'diagnose' }, lvArg())).then(function (r) {
            setDiagBusy(false);
            if (r && r.ok !== false) setLogDiag(r);
            else setErr((r && r.error) || '试玩体检失败');
          });
        };
        ...
        /** 只列目录（不截图）。 */
        var doShotList = function () {
          callTool('miliastra_shot', { op: 'list' }).then(function (r) {
            if (r && r.ok !== false) setShotInfo(r);
          });
        };
        ...
        /*
         * 「试玩体检」结论条 —— 放在日志卡片**最上面**。
         * 只在「最近一局不新鲜 / 一个日志都没有」时用**醒目样式**弹出来；
         * fresh 时只留一行淡色说明（证明工具确实看过现场，不是没查）。
         */
        if (logDiag) {
          var fresh = logDiag.verdict === 'fresh';
          logKids.push(React.createElement('div', {
            className: fresh ? PLUGIN + '-hint' : PLUGIN + '-err',
            key: 'diag',
          },
            React.createElement('div', null, plain(logDiag.headline)),
            !fresh && (logDiag.why || []).length
              ? React.createElement('div', { style: { marginTop: '4px' } },
                React.createElement('b', null, '可能原因：'),
                React.createElement('div', null, (logDiag.why || []).map(plain).join('　')))
              : null,
            !fresh && (logDiag.next || []).length
              ? React.createElement('div', { style: { marginTop: '4px' } },
                React.createElement('b', null, '下一步：'),
                React.createElement('div', null, (logDiag.next || []).map(plain).join('　')))
              : null));
        }
        logKids.push(React.createElement('div', { className: PLUGIN + '-row', key: 'diagbtn' },
          React.createElement('button', {
            className: PLUGIN + '-act', disabled: diagBusy || busy, key: 'dg',
            title: '「我刚试玩了，为什么没有日志？」—— 把现场证据摆出来给结论',
            onClick: doDiagnose,
          }, diagBusy ? '体检中…' : '试玩体检')));
        ...
        if (shotInfo && (shotInfo.files || []).length) {
          shotKids.push(React.createElement('div', { className: PLUGIN + '-row', key: 'fl' },
            React.createElement('button', {
              className: PLUGIN + '-act', style: { padding: '2px 9px', fontSize: '10.5px' }, key: 't',
              onClick: function () { setShowShotFiles(!showShotFiles); },
            }, (showShotFiles ? '收起' : '展开') + '最近 ' + (shotInfo.files || []).length + ' 张')));
          if (showShotFiles) {
            shotKids.push(React.createElement('div', { className: PLUGIN + '-log', key: 'files', style: { maxHeight: 160 } },
              (shotInfo.files || []).map(function (f) {
                return React.createElement('div', { key: f.name },
                  f.name + '　' + (f.sizeText || '') + (f.mtime ? '　' + hhmm(f.mtime) : ''));
              })));
          }
        }
