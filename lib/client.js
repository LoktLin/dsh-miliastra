  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      var CSS = [
        ...
        // —— 模拟器：③ AI 试玩区域（下面那块大长方形：试玩画面 + 传输控制 + 时间线） ——
        '.' + PLUGIN + '-simai{display:flex;flex-direction:column;gap:9px;margin:0 13px 13px;padding:10px 11px;',
        'border:1px solid rgba(125,211,252,.20);border-radius:12px;background:rgba(125,211,252,.05);flex:0 0 auto;}',
        '.' + PLUGIN + '-simai>.' + PLUGIN + '-col-head{display:flex;align-items:center;gap:8px;flex-wrap:wrap;}',
        '.' + PLUGIN + '-inline .' + PLUGIN + '-simai img{max-height:60vh;}',
        /*
         * ⚠️ 面板是 `overflow:hidden`、`-body` 也是 —— 「AI 试玩区域」是 `-body` 的**兄弟**，
         * 不把 `-simbody` 变成滚动容器的话，下面那块会被直接切掉（渲染得出来、但点不到）。
         * 所以：模拟器页**整体一起滚**（列各自的内滚在这里让位，避免"滚中滚"）。
         */
        '.' + PLUGIN + '-simbody{display:flex;flex-direction:column;flex:1 1 auto;min-height:0;overflow-y:auto;}',
        '.' + PLUGIN + '-simbody::-webkit-scrollbar{width:8px;}',
        '.' + PLUGIN + '-simbody::-webkit-scrollbar-thumb{background:rgba(125,211,252,.28);border-radius:8px;}',
        '.' + PLUGIN + '-simbody>.' + PLUGIN + '-body{flex:0 0 auto;overflow:visible;max-height:none;}',
        '.' + PLUGIN + '-simbody>.' + PLUGIN + '-body>.' + PLUGIN + '-col{overflow:visible;max-height:none;}',
      ...
      function SimulatorBody() {
        ...
        var stage = shot
          ? React.createElement('img', {
            src: shot.url, alt: 'sim',
            title: '点画布上任意位置 = 往引擎里注入一次点击（坐标按「左下原点」换算）',
            style: { width: '100%', borderRadius: '6px', background: '#0b1220', cursor: 'crosshair' },
            onClick: function (e) {
        ...
        var left = React.createElement('div', { className: PLUGIN + '-col', key: 'L' },
          React.createElement('div', { className: PLUGIN + '-col-head', key: 'h' }, '① 工程与控件树'),
        ...
        /**
         * ② 画面与日志：**横排**（2026-09-24 按作者批注改）。
         * 原来是上下叠着的：要"现在长什么样"和"日志说了什么"来回扫。横排后一眼同时看得到。
         * 这里只是**缩略** —— 真正的大画面在下面的「AI 试玩区域」。
         */
        var thumb = shot
          ? React.createElement('img', {
            src: shot.url, alt: 'sim-thumb',
            style: { width: '100%', maxHeight: '132px', objectFit: 'contain', borderRadius: '6px', background: '#0b1220' },
          })
          : React.createElement('div', { className: PLUGIN + '-hint' }, '还没有画面 —— 点「编辑器画面」，或先「开始试玩」再点「试玩画面」。');

        var right = React.createElement('div', { className: PLUGIN + '-col', key: 'R' },
          ...
          React.createElement('div', { className: PLUGIN + '-simrow', key: 'row' },
            sec('画面', shot ? shot.name : '未取图',
              React.createElement('div', { key: 's' }, thumb), 'simStage', shot ? 'ok' : 'warn'),
        ...
        /**
         * ③ **AI 试玩区域**（作者批注：「下面的长方形区域当作 AI 试玩区域」）——
         * 把"能试玩的东西"收在一处：试玩画面（点画布 = 往引擎注入一次点击）＋ 传输控制（开始 / 单步 / 连帧 / 停止）＋
         * 按键与设备·人数·视角 ＋ **这一局的操作时间线**（AI 用 `op=verify fromHistory:true` 复用的就是它）。
         * 面板与人共用同一个会话，所以人在这里点的每一步，AI 那边都看得到。
         */
        var aiArea = React.createElement('div', { className: PLUGIN + '-simai', key: 'ai' },
          React.createElement('div', { className: PLUGIN + '-col-head', key: 'h' },
            React.createElement('span', { key: 't' }, '③ AI 试玩区域'),
            React.createElement('span', { className: PLUGIN + '-hint', key: 'i' },
              (running ? '会话运行中' : '未开始')
              + ' · 帧 ' + statFrame
              + ' · 控件 ' + statControls
              + ' · 画布 ' + ((st && st.canvas && st.canvas.label) || '—'))),
          actions, keyRow, devRow,
          React.createElement('div', { key: 'stage' }, stage),
          sec('操作时间线', hist.length ? '最近 ' + hist.length + ' 步' : '空',
            React.createElement('div', { className: PLUGIN + '-log', key: 'h', style: { maxHeight: '110px' } },
              hist.length
                ? hist.join('\n')
                : '（人在这里点 / 按键之后，这里会出现可重放的操作序列 —— AI 用 op=verify fromHistory:true 直接把这一局变成回归用例）'),
            'simHist', hist.length ? 'ok' : 'warn'));

        kids.push(React.createElement('div', { className: PLUGIN + '-body', key: 'body' }, [left, right]));
        kids.push(aiArea);
