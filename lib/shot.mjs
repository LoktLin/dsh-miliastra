 ...
 * 所以这里把「疑似抓错」显式变成返回值的一部分，而不是留给运气。
 */
export function judgeCapture(r) {
  if (!r || r.ok === false) return { suspect: true, warning: r && r.error ? String(r.error) : '截图失败' };
  const ratio = Number(r.blackRatio);
  const warn = [];
  let suspect = false;

  if (Number.isFinite(ratio) && ratio > 0.985) {
    suspect = true;
    warn.push(`画面几乎全黑（黑像素 ${(ratio * 100).toFixed(1)}%）—— 目标窗口很可能没渲染出内容，`
      + '或者它最小化/被遮挡。');
  }
  if (r.mode === 'screen') {
    if (r.front === false) {
...
/** 真删。返回实际删掉的与失败的，**不抛**。 */
export function removeShots(paths) {
  const removed = [];
  const failed = [];
  for (const p of paths || []) {
    try { fs.unlinkSync(p); removed.push(p); } catch (e) { failed.push({ path: p, error: (e && e.message) || String(e) }); }
  }
 ...
 *                    front?:boolean, blackRatio?:number, pid?:number, process?:string,
 *                    title?:string, error?:string, stderr?:string}>}
 */
export function captureWindow({
  processName, out, bringToFront = 1, keepWindowOnTop = 0, timeoutMs = 60000, script = CAPTURE_SCRIPT,
} = {}) {
