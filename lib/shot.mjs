...
export function sanitizeLabel(raw, maxLen = 24) {
  ...
  // ⚠️ **截断之后要再剥一次首尾点/横线**：先剥后截的话，截断处又会留下一个尾点
  //    （实测：`selftest-move-frames-t0.5` 截成 24 字符后变成 `…-frames-t0.` —— 2026-09-24 踩到）
  s = s.replace(/^[.\-]+|[.\-]+$/g, '');
  return s;
}
 ...
 *
 * @param {{files?: Array<{path:string,name:string,size:number,mtimeMs:number}>, keepLast?: number,
 *          olderThanDays?: number, all?: boolean, now?: number}} [opts]
 *        `files` 全部截图；`keepLast` 至少保留最新的 N 张（保护网，任何模式下都生效）；
 *        `olderThanDays` 只删比这个更旧的（>0 才生效）；`all` 不管新旧、除 keepLast 外全删；
 *        `now` 是判定用的当前时刻（默认 `Date.now()`，测试里注入固定值）
 ...
 *
 * ── P2-2（2026-09-26）：+ 两条**能判就判**的启发式 ─────────────────────────
 * 实测那次：`target=game` 的回执是 `process:YuanShen / pid / title:原神`，画出来却是编辑器那一屏 ——
 * 光看 `ok:true` + 齐全的 pid/title **发现不了**。这里只加**判得出来**的东西（不编"画面内容识别"）：
 *   · ⑤ **进程对不上**：请求的进程名 ≠ 截图脚本回报的进程名；
 *   · ⑥ **标题像编辑器**：要的是 game，而选中窗口的标题命中编辑器/沙箱类关键词。
 * 判不出来的（比如"游戏进程里显示的是编辑器界面"这种内容级差异）**不假装判** ——
 * 回执改为把**候选窗口清单**（标题/尺寸/是否最小化/选中哪个）与 `mode`/`front`/黑与单色比例一起给出。
 */
export function judgeCapture(r, { target = null, requestedProcess = null } = {}) {
  ...
  /* ⑤ 进程对不上（请求 game，截图脚本却回报了另一个进程名） */
  const wantProc = requestedProcess ? String(requestedProcess).replace(/\.exe$/i, '') : '';
  const gotProc = r.process ? String(r.process).replace(/\.exe$/i, '') : '';
  if (wantProc && gotProc && wantProc.toLowerCase() !== gotProc.toLowerCase()) {
    suspect = true;
    warn.push('请求的是进程 `' + wantProc + '`，而截图脚本回报的是 `' + gotProc + '` —— 这次截的**不是你要的那个程序**。');
  }

  /* ⑥ target=game 但选中窗口的标题像编辑器/沙箱（实测那次标题是「原神」，所以这条只挡得住一部分） */
  const title = String(r.title || '');
  const EDITORISH = /(BeyondEditor|沙箱|编辑器|界面控件组|高级数据管理|节点图|资源管理器)/i;
  if (target === 'game' && EDITORISH.test(title)) {
    suspect = true;
    warn.push('要的是**游戏画面**，而选中窗口的标题是 `' + title + '` —— 像是编辑部 / 沙箱界面。'
      + '（这条只能凭标题判：**画面内容本身本工具不识别**，请用 candidates 里的标题/尺寸确认，或直接看图。）');
  }

  // 候选窗口清单本身不是"可疑"，但它是"选错了"唯一能被人/AI 看出来的依据 —— 一并提示怎么看
  const cands = Array.isArray(r.candidates) ? r.candidates : [];
  if (cands.length > 1) {
    warn.push('这个进程有 ' + cands.length + ' 个可见窗口，`candidates` 逐条给了标题/尺寸/是否最小化，'
      + '并标出了本次选中的那个；选错时用 `window=<标题子串>` 指定。');
  }
  return { suspect, warning: warn.length ? warn.join(' ') : null };
}

/**
 * 把 PS 侧回报的候选窗口清单标出「哪个被选中了」（**纯函数**，便于单测）。
 *
 * 为什么要标：`candidates` 本来就给了标题/尺寸/`minimized`，但**没有"选中哪个"** ——
 * 一眼看不出这次到底截的是哪一条（实测踩过：进程多窗口时抓到了 160×28 的标题栏碎片）。
 *
 * @param {Array} candidates PS 回报的候选（`{pid,title,w,h,area,minimized}`）
 * @param {{pid?:any, title?:any, width?:any, height?:any}} chosen 实际截到的那个窗口
 * @returns {Array<{pid:any,title:string,width:number|null,height:number|null,area:number|null,minimized:boolean,selected:boolean}>}
 */
export function markSelectedCandidate(candidates, chosen = {}) {
  const list = Array.isArray(candidates) ? candidates : [];
  const cp = chosen.pid == null ? null : String(chosen.pid);
  const ct = chosen.title == null ? null : String(chosen.title);
  const cw = Number(chosen.width);
  const ch = Number(chosen.height);
  return list.map((c) => {
    const same = (cp != null && String(c.pid) === cp)
      && (ct != null ? String(c.title || '') === ct : true)
      && (!Number.isFinite(cw) || Number(c.w) === cw)
      && (!Number.isFinite(ch) || Number(c.h) === ch);
    return {
      pid: c.pid,
      title: c.title || '',
      width: Number.isFinite(Number(c.w)) ? Number(c.w) : null,
      height: Number.isFinite(Number(c.h)) ? Number(c.h) : null,
      area: Number.isFinite(Number(c.area)) ? Number(c.area) : null,
...
/**
 * 这一张**是不是在那一局的窗口里**拍的 —— **纯函数**（反馈 C1 ③）。
 *
 * 为什么要有它（2026-09-25 同事实测）：一局只有 16 秒，而「命中后留 8 秒加载窗口再连拍」的 4 张
 * **全部落在局外**（`startedAt 16:34:21.804` / `endedAt 16:34:37.521`，而 4 张正片拍于 16:34:38~16:34:49）——
 * 但回执里只有一堆 `elapsedMs`，**得自己算**才知道全废了。这一条就是替 AI 把这笔账算掉。
 *
 * @param {number} atMs 这张图的拍摄时刻
 * @param {{startedAtMs?:number|null, endedAtMs?:number|null}|null} run 那一局的开跑/结束时刻
 * @returns {boolean|null} 窗口未知时返回 `null`（**不猜**）
 */
export function frameInRun(atMs, run) {
  if (!run || !Number.isFinite(run.startedAtMs) || !Number.isFinite(atMs)) return null;
  if (atMs < run.startedAtMs) return false;
  if (Number.isFinite(run.endedAtMs) && atMs > run.endedAtMs) return false;
  return true;
}
 ...
 * @param requestedMs 请求的帧间隔（用来标 `clamped`）
 * @param run 那一局的窗口 `{startedAtMs, endedAtMs}` —— 给了就逐张标 `inRun`（反馈 C1 ③）
 */
export function burstSummary(frames, { startedAtMs = null, requestedMs = null, run = null } = {}) {
  ...
  const window = run && Number.isFinite(run.startedAtMs)
    ? { startedAtMs: run.startedAtMs, endedAtMs: Number.isFinite(run.endedAtMs) ? run.endedAtMs : null }
    : null;
  const rows = list.map((f) => {
    // ★ 逐张标「在不在本局窗口内」：调用方算过就给调用方的，否则按窗口算；窗口未知 → null（不猜）
    const inRun = typeof f.inRun === 'boolean' ? f.inRun : (window ? frameInRun(f.atMs, window) : null);
    return {
      i: f.i,
      file: f.file || null,
  ...
  const inRunCount = rows.filter((r) => r.inRun === true).length;
  const outsideCount = rows.filter((r) => r.inRun === false).length;
  return {
    count: list.length,
    ...
    note: '图走 `GET /miliastra/shot?name=<file>` 看原图、`&thumb=1` 看小图；'
      ...
      + (window
        ? ' 逐张的 `inRun` = **这张是不是在本局窗口内**（' + inRunCount + ' 张在内 / ' + outsideCount + ' 张在外）'
          + '— 窗口外的图**别当证据**（多半是加载画面或局已结束）。'
        : '')
 ...
 *
 * @param {{src?: string, out?: string, width?: number, timeoutMs?: number, script?: string}} [opts]
 *        `src` 原图路径、`out` 预览落点 —— 这两个**调用时必须给**（签名带 `= {}` 所以标成可选，
 *        但真省了就只会拼出 `-In undefined -Out undefined`，那是调用错误）；
 *        `width` 预览宽度（默认 320）；`timeoutMs` 起 PowerShell 的超时（默认 40000）；
 *        `script` 覆盖 ps1 路径
 */
 ...
 *
 * @param {{processName?: string, out?: string, title?: string, thumbOut?: string, thumbWidth?: number,
 *          bringToFront?: number, keepWindowOnTop?: number, timeoutMs?: number, script?: string}} [opts]
 *        `processName` 进程名（不带 .exe）、`out` 输出 PNG 绝对路径 —— 这两个**调用时必须给**
 *        （签名带 `= {}` 所以标成可选，但真省了只会拼出 `-ProcessName undefined`，那是调用错误）；
 *        `title` 按窗口标题子串挑窗口（进程有多个窗口时用）；`thumbOut` 顺带生成一张预览
 *        （同一个 bitmap，不额外起进程）、`thumbWidth` 预览宽度；`bringToFront` / `keepWindowOnTop`
 *        是传给 PS 的 0/1 开关（默认 1 / 0）；`timeoutMs` 起 PowerShell 的超时；`script` 覆盖 ps1 路径
