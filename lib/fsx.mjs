...
export function atomicWriteFile(dest, buf) {
  ...
  try {
    ...
    /*
     * ⚠️ 这里**必须**用 `unlinkSync`，不能用 `rmSync(tmp, { force: true })`。
     *    实测（2026-09-25 同事的最小复现）：**DSH 打包版 Node v24.9.0** 上
     *    `fs.rmSync(p, { force: true })` **不报错、正常返回，而文件仍然存在** ——
     *    于是「清理临时文件」这一步静默失效，写失败会在盘上留下 `.tmp-` 残留
     *    （同环境 `unlinkSync` 正常；系统版 Node v24.19.0 两者都正常，所以本机看不出来）。
     *    `unlinkSync` 对单个文件的语义明确，删不掉会抛错 —— 而这里包了 try/catch：
     *    真删不掉也只是残留一个临时文件，**不会掩盖上面那个真正要抛的异常**。
     */
